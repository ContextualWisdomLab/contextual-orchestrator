"""Durable multi-provider discovery and runtime-agent construction.

The provider catalog converts a fixed deploy-time credential inventory into an
auditable runtime model pool. Provider API-key values are transported into the
existing credential registry by a trusted bootstrap process and are resolved by
name at request time. Catalog tables store provider accounts and model metadata,
never raw or encrypted provider-secret values.

Refreshes are account-scoped. A failed provider refresh records a stable error
code and preserves that account's last-known-good model set. A first deployment
with no usable chat-capable model fails closed instead of silently starting a
mock or empty pool.
"""
...
"""Maximum accepted bytes in one provider model-catalog or native response."""

MAX_RETRY_AFTER_SECONDS = 30.0
"""Maximum delay honored from an untrusted provider Retry-After header."""
...
"""
"""Third-normal-form PostgreSQL schema for provider catalog metadata."""
...
class ProviderCatalogStore(Protocol):
    ...
    def enabled_models(self) -> list[CatalogModelRecord]:
        """Return enabled models that belong to enabled provider accounts."""
        ...
...
class PostgresProviderCatalogStore:  # pragma: no cover - live database adapter
    ...
    def discover(self, account: ProviderAccount, credential: str) -> list[DiscoveredModel]:
        ...
        for attempt in range(self.max_attempts):
            ...
            try:
                ...
                delay = min(MAX_RETRY_AFTER_SECONDS, remaining, max(0.0, requested))
                if delay:
                    self._sleep(delay)
...
class ProviderAwareModelClient(ModelClient):
    """Use the hardened OpenAI transport plus a narrow native Bytez adapter."""
    ...
    def chat(self, agent: ModelAgent, messages: list[dict[str, str]], temperature: float = 0.2) -> str:
        ...
        document = self._bytez_request(agent, prompt, credential)
    ...
    def stream_chat(self, agent: ModelAgent, messages: list[dict[str, str]], temperature: float = 0.2):
        """Frame a completed Bytez answer when no verified native SSE contract exists."""
        if agent.provider_name != "bytez":
...
class ProviderCatalogService:
    ...
    def candidate_agents(self) -> list[ModelAgent]:
        """Convert chat-capable catalog rows into role-tagged model agents."""
...
def bootstrap_provider_credentials(
    ...
    *,
    ...
    Required bootstrap validates the entire fixed inventory before mutation,
...
def _normalize_bytez_output(document: Mapping[str, Any]) -> str:
    ...
    candidates.extend(
        document.get(name) for name in ("content", "text", "generated_text")
    )
...
def _parse_retry_after(value: str | None, *, now: datetime | None = None) -> float | None:
    """Parse and cap Retry-After delta-seconds or HTTP-date values."""
    if value is None:
        return None
    token = value.strip()
    if not token:
    ...
    return min(MAX_RETRY_AFTER_SECONDS, max(0.0, (parsed - reference).total_seconds()))
...
def _secure_json_request(  # pragma: no cover - real credentialed network boundary
    *,
    ...
    for address in addresses:
        connection = _PinnedCatalogConnection(parsed.hostname, address, port, timeout_seconds)
        try:
...
def _validated_global_addresses(hostname: str, port: int) -> tuple[str, ...]:  # pragma: no cover
...
def _infer_capabilities(
    ...
    if declared:
        ...
        specialized: tuple[tuple[tuple[str, ...], str], ...] = (
            ...
            (("dall-e", "image-generation", "stable-diffusion", "flux"), "image_generation"),
...
def _model_priority(model: DiscoveredModel) -> int:
    ...
    price_tie = 0
    if known_prices and sum(known_prices) / len(known_prices) < 1.0:
        price_tie = 1
...
def main(argv: Sequence[str] | None = None) -> int:  # pragma: no cover - CLI integration
    ...
    if args.command in {"bootstrap-and-sync", "sync"}:
        ...
        candidates = service.candidate_agents()
        catalog_summary = {
            "candidate_model_count": len(candidates),
            "provider_accounts": {},
        }
