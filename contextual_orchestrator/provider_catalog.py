...
class ProviderAwareModelClient(ModelClient):
    ...
    def chat(self, agent: ModelAgent, messages: list[dict[str, str]], temperature: float = 0.2) -> str:
        ...
        last_error: CatalogHttpError | None = None
        for attempt in range(self.max_retries + 1):
            try:
                return _normalize_bytez_output(self._bytez_request(agent, prompt, credential))
            except CatalogHttpError as exc:
                last_error = exc
        ...
        if last_error is not None:
            raise ProviderCatalogUnavailable(
                f"Bytez request failed with {last_error.code}"
            ) from None
        raise ProviderCatalogUnavailable("Bytez request failed")
    ...
    def _request_bytez(self, agent: ModelAgent, prompt: str, credential: str) -> Mapping[str, Any]:  # pragma: no cover
        ...
        return secure_json_request(
            method="POST",
            url=f"{agent.base_url.rstrip('/')}/models/v2/{_bytez_model_path(agent.model)}",
...
def _bytez_model_path(model_name: str) -> str:
    """Encode a Bytez namespace/model id without permitting path traversal."""
    segments = model_name.split("/")
    if not segments or any(not segment or segment in {".", ".."} for segment in segments):
        raise ProviderCatalogUnavailable("Bytez model id contains an invalid path segment")
