...
ADMIN_TRANSLATIONS = {
    "en": {
        ...
        "session_hint": "Establish an HttpOnly admin session (opaque server-side id, not the raw bearer) so browser calls use a cookie instead of embedding the token in JavaScript. Reverse proxies may inject Authorization instead.",
    ...
    "ko": {
        ...
        "session_hint": "HttpOnly 관리자 세션(원시 bearer가 아닌 서버측 불투명 식별자)을 열면 브라우저 호출이 JavaScript에 토큰을 저장하지 않고 쿠키를 사용합니다. 리버스 프록시는 Authorization 헤더를 주입할 수 있습니다.",
