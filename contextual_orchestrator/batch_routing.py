...
class ProviderEmbeddingBatchBackend:
    ...
    def wait(self, job: BatchJob, *, timeout: float | None) -> Dict[str, Any]:
        """Wait until terminal state or an explicit timeout.

        ``None`` and infinity both mean no application deadline; neither can
        be passed through ``math.isfinite`` to ``threading.Event.wait``.
        """
        event = self._terminal_events.get(job.job_id)
        if event is not None:
            event.wait(
                timeout=timeout if timeout is not None and math.isfinite(timeout) else None
            )
