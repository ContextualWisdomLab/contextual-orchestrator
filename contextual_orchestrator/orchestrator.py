...
class ModelClient:
    ...
    def _build_ssl_context(ca_bundle: str | None, verify_tls: bool) -> ssl.SSLContext:
        if not verify_tls:
            return ssl._create_unverified_context()  # nosec B323 - reachable only via the explicit verify_tls=False dev-only opt-out documented above; default path verifies. nosemgrep: python.lang.security.unverified-ssl-context.unverified-ssl-context
        if ca_bundle:
    ...
    def _open_provider(self, request: urllib.request.Request) -> Any:
        ...
        return urllib.request.urlopen(  # nosec B310 - URL is built by _provider_url (scheme/path checks) after _validate_provider https+allowlist+SSRF checks. nosemgrep: python.lang.security.audit.dynamic-urllib-use-detected.dynamic-urllib-use-detected
            request,
