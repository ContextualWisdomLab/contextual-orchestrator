...
class TaskOrchestrator:
    ...
    def conduct(
        ...
        *,
        ...
        review_free_pool = bool(
            free_ids and all("review" in self._agent(agent_id).tags for agent_id in free_ids)
        )
        ...
        if review_free_pool and model_name == self.FREE_MODEL:
            result["request_admitted_ids"] = [
                agent.id for agent in self.agents if agent.id in free_ids
            ]
            result["review_served_steps"] = [
