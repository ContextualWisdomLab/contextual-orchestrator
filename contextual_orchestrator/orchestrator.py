...
def text_completion_response(
    ...
    Older SDKs and batch tools still call Completions with ``prompt`` instead of
    chat ``messages``. The gateway maps prompt → a single user turn and returns
    the classic ``choices[].text`` shape so those clients keep working.
    """
    orchestration = {
        "workflow_run_id": result.get("workflow_run_id"),
        "mode": result["mode"],
