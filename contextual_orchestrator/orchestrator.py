...
class TaskOrchestrator:
    ...
    def complete(
        ...
        *,
        ...
        ``preferred_model`` is the client-requested OpenAI ``model`` id. When it
        matches an enabled agent model, that agent is preferred for the worker
        route; the gateway default falls back to capability ranking.
        """
    ...
    def _ranked_agents(
        ...
        *,
        ...
        return sorted(
            self.agents,
            key=lambda agent: self._score_agent(agent, role, lowered, preferred_model=preferred_model),
            reverse=True,
        )
