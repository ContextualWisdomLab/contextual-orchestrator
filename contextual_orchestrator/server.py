...
def _validate_completions_stop(body: dict[str, Any]) -> str | list[str] | None:
    ...
    stop sequences on the Completions path, so any provided non-empty ``stop`` fails closed. Empty array/null are omit no-ops.
    """
    if "stop" not in body:
        return None
    stop = body.get("stop")
            ...
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    function_call_present = (
                        "function_call" in body and function_call_raw is not None
                    )
                    ...
                    if "logit_bias" in body:
                        ...
                        try:
                            ...
                            raise
                    if "stop" in body:
                        # Explicit JSON null or empty [] is treat-as-omit (SDK optional default).
                        stop_val = body.get("stop")
                        if stop_val is not None and stop_val != []:
                            try:
                                _validate_completions_stop(body)
                            except RequestError as exc:
