...
def _validate_completions_frequency_penalty(body: dict[str, Any]) -> float | None:
    ...
    Applied to the provider client for this call when in range. Booleans and
    out-of-range values fail closed so buyers never believe a discarded penalty
    was honored.
    """
...
def _validate_completions_temperature(body: dict[str, Any]) -> float | None:
    ...
    Applied to the provider client for this call when in range. Booleans and
    out-of-range values fail closed (OpenAI parity on Completions sampling).
    """
...
def _validate_completions_top_p(body: dict[str, Any]) -> float | None:
    ...
    Applied to the provider client for this call when in range. Zero, negatives,
    values above 1, and booleans fail closed.
    """
...
def _validate_completions_top_logprobs(body: dict[str, Any]) -> None:
    ...
    OpenAI Completions uses integer ``logprobs`` (0–5) for alternate-token counts;
