...
ALLOWED_COMPLETIONS_KEYS = {
    ...
    "presence_penalty", "frequency_penalty", "logit_bias", "metadata",
} | {"attribution", "routing"}
...
def _validate_chat_metadata(body: dict[str, Any]) -> dict[str, str] | None:
    """OpenAI chat Completions ``metadata`` — object of string pairs, max 16.

    Keys at most 64 characters; values at most 512 characters. Non-objects and
    non-string entries fail closed so clients cannot store untyped junk that
    cost/observability consumers would silently drop.
    """
