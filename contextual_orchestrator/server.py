...
def _validate_completions_logit_bias(body: dict[str, Any]) -> dict[str, float] | None:
    ...
    (digit keys, values in [-100, 100], ≤300 entries). An empty object applies no
    bias and is accepted; any non-empty map is rejected because biases are not
    applied on the Completions route path.
    """
    ...
    # Empty object is a no-op (no token biases to apply); accept for honesty.
    if len(bias) == 0:
        return {}
    if len(bias) > 300:
...
def _validate_chat_stream_options(body: dict[str, Any], stream: bool) -> dict[str, Any] | None:
    """Chat Completions ``stream_options`` — requires stream=true; empty/false flags ok.

    Shape matches OpenAI (include_usage / include_obfuscation booleans). An empty
    object and ``false`` flags are no-ops and accepted. This gateway's SSE path
    does not emit a final usage chunk and does not apply stream obfuscation, so
    include_usage/include_obfuscation=true fail closed.
    """
    if "stream_options" not in body:
        return None
    opts = body.get("stream_options")
            ...
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    if "logit_bias" in body:
                        try:
                            # Empty {} is a no-op and accepted; non-empty fails closed.
                            _validate_completions_logit_bias(body)
                        except RequestError as exc:
