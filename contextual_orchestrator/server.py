            ...
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    if "parallel_tool_calls" in body:
                        # Parallel tool invocation is only meaningful with tools and is
                        # handled on the passthrough path. Outside passthrough, accept
                        # false/omit; reject true so clients cannot believe fan-out ran.
                        ...
                        if ptc is True:
                            raise RequestError(
                                400,
                                "invalid_parallel_tool_calls",
                                "parallel_tool_calls=true is not supported on /v1/chat/completions "
                                "without tools; include tools for provider passthrough",
                            )
                    started_at = time.perf_counter()
                    model_client = orchestrator.client
                    previous_max_tokens = model_client.max_output_tokens
