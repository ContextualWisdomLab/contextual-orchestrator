...
_MAX_COMPLETIONS_STOP_SEQUENCES = 4
...
def _validate_completions_stop_sequences(body: dict[str, Any]) -> list[str] | None:
    """Legacy Completions ``stop`` — string or array of up to 4 non-empty strings."""
    if "stop" not in body:
        return None
    stop = body.get("stop")
    ...
    if isinstance(stop, list):
        if not stop:
            raise RequestError(400, "invalid_stop", "stop array must be non-empty")
        if len(stop) > _MAX_COMPLETIONS_STOP_SEQUENCES:
            raise RequestError(
                400,
                "invalid_stop",
                f"stop array may contain at most {_MAX_COMPLETIONS_STOP_SEQUENCES} sequences",
                {"count": len(stop), "max_count": _MAX_COMPLETIONS_STOP_SEQUENCES},
            )
        validated: list[str] = []
        ...
        def do_POST(self) -> None:  # noqa: N802
            try:
                ...
                if path == "/v1/completions":
                    ...
                    _validate_completions_stop_sequences(body)
