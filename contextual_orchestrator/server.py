...
def _validate_completions_suffix(body: dict[str, Any]) -> str | None:
    ...
    # OpenAI empty string is a no-op; any non-empty content (including whitespace)
    # would be inserted after the completion — this gateway never does that, so
    # fail closed rather than silently dropping the suffix.
    if suffix != "":
        raise RequestError(
            400,
            "invalid_suffix",
