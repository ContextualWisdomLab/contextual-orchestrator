...
def _validate_completions_logit_bias(body: dict[str, Any]) -> dict[str, float] | None:
    """Legacy Completions ``logit_bias`` — type-checked then rejected (not applied).

    OpenAI uses logit_bias to bias token sampling. This gateway validates map shape
    (digit keys, values in [-100, 100], ≤300 entries) but does not apply biases on
    the Completions route path, so any provided ``logit_bias`` fails closed.
    """
...
def _validate_chat_max_completion_tokens(body: dict[str, Any]) -> int | None:
    """Chat Completions ``max_completion_tokens`` — positive integer, same cap as max_tokens.

    OpenAI deprecates ``max_tokens`` in favor of ``max_completion_tokens`` for chat.
    This gateway applies the value to the provider client budget when provided.
    """
...
def _validate_chat_logprobs(body: dict[str, Any]) -> None:
    """Chat Completions ``logprobs`` / ``top_logprobs`` — type-check then fail closed.

    OpenAI chat uses boolean ``logprobs`` plus optional integer ``top_logprobs`` (0–20).
    This gateway always returns ``logprobs: null`` on the chat route path, so
    ``logprobs=true`` and any provided ``top_logprobs`` fail closed. ``logprobs=false``
    and omit remain valid.
    """
    if "logprobs" in body:
        logprobs = body.get("logprobs")
        ...
        if logprobs is True:
            raise RequestError(
                400,
                "invalid_logprobs",
                "logprobs is not supported on /v1/chat/completions",
            )
                ...
                if path == "/v1/chat/completions":
                    ...
                    if isinstance(user_val, str) and user_val.strip():
                        ...
                        if not attribution.get("account"):
                            attribution["account"] = user_val
                    elif "user" in body and user_val is not None:
                        raise RequestError(400, "invalid_user", "user must be a non-empty string of at most 64 characters")
                    if model_name and not attribution.get("model_name"):
                        attribution["model_name"] = model_name
                    if not attribution.get("service"):
                    ...
                    if "max_completion_tokens" in body:
                        mct = _validate_chat_max_completion_tokens(body)
                        if max_tokens is not None and mct is not None and max_tokens != mct:
                            raise RequestError(
                                400,
                                "invalid_max_completion_tokens",
                                "max_completion_tokens must match max_tokens when both are set",
                            )
