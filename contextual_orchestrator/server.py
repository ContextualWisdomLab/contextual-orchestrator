...
def _validate_responses_logit_bias(body: dict[str, Any]) -> dict[str, float] | None:
    """Responses ``logit_bias`` — shape-check then pass through valid token maps.

    OpenAI biases token sampling via digit-id keys and numeric values in [-100, 100].
    Responses passthrough forwards well-formed maps to the provider. Empty ``{}`` is
    an honest no-op. Invalid types/keys/ranges fail closed so clients never believe a
    broken bias map was applied.
    """
    if "logit_bias" not in body:
        return None
    bias = body.get("logit_bias")
        ...
        def do_POST(self) -> None:  # noqa: N802
            try:
                ...
                if path == "/v1/responses":
                    ...
                    # Token budgets: shape/range fail-closed before provider passthrough.
                    # Valid values are forwarded; invalid types/ranges must not 200.
                    if "max_completion_tokens" in body:
