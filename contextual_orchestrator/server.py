...
def _validate_completions_tools_surface(body: dict[str, Any]) -> None:
    ...
    """
    if any(key in body for key in ("tools", "tool_choice", "functions", "function_call", "parallel_tool_calls")):
        raise RequestError(
            400,
            "invalid_tools",
...
def _validate_openai_background(body: dict[str, Any], *, endpoint_path: str) -> None:
    """Reject OpenAI ``background`` async mode — this gateway is request-scoped.

    OpenAI may run long jobs asynchronously when ``background=true``. This
    gateway has no background job plane; fail closed with a named error.
    """
    ...
    raise RequestError(
        400,
        "invalid_background",
        f"background is not supported on {endpoint_path}",
    )



def _validate_chat_include_field(body: dict[str, Any]) -> None:
    """Reject OpenAI chat ``include`` — Responses conversation-control surface only.

    Some SDKs send ``include`` on chat Completions. This gateway only validates
    ``include`` on /v1/responses (and rejects it there as unsupported). Named
    error beats opaque unknown_fields on chat.
    """
