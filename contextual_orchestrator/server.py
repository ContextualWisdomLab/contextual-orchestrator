            ...
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    if "stream_options" in body:
                        # Mirrors OpenAI: stream_options is only valid with stream=true.
                        # Non-stream requests fail closed so include_usage cannot be silently ignored.
                        opts = body.get("stream_options")
                        if not isinstance(opts, dict):
                            raise RequestError(
                                400,
                        ...
                        if "include_obfuscation" in opts and not isinstance(
                            opts["include_obfuscation"], bool
                        ):
                            raise RequestError(
                                400,
                                "invalid_stream_options",
