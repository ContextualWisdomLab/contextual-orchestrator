...
def _validate_completions_tools_surface(body: dict[str, Any]) -> None:
    ...
    Empty ``tools: []`` is treated as omit (SDK no-op). Non-empty tools or any
    tool_choice/functions/function_call/parallel_tool_calls fail closed.
    """
    tools = body.get("tools") if "tools" in body else None
    if isinstance(tools, list) and not tools:
        # Honest no-op: empty tools array means no tools were configured.
        tools_present = False
    else:
        tools_present = "tools" in body
    if tools_present or any(
        key in body for key in ("tool_choice", "functions", "function_call", "parallel_tool_calls")
    ):
        raise RequestError(
            400,
            "invalid_tools",
        ...
        def do_POST(self) -> None:  # noqa: N802
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    if "tool_choice" in body and not tools_list:
                        # tool_choice alone / empty tools: only "none" is a valid no-op.
