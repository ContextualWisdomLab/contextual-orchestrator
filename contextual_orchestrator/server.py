...
_MAX_COMPLETIONS_USER_CHARS = 64  # OpenAI end-user id hard cap
...
def _validate_completions_user(body: dict[str, Any]) -> str | None:
    """Legacy Completions ``user`` — non-empty string, at most 64 characters when present."""
    if "user" not in body:
        return None
    user = body.get("user")
    if not isinstance(user, str) or not user.strip():
        raise RequestError(400, "invalid_user", "user must be a non-empty string")
    if len(user) > _MAX_COMPLETIONS_USER_CHARS:
        raise RequestError(
            400,
            "invalid_user",
            f"user must be at most {_MAX_COMPLETIONS_USER_CHARS} characters",
            {"length": len(user), "max_length": _MAX_COMPLETIONS_USER_CHARS},
        )
    return user
