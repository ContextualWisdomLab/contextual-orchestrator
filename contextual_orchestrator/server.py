        ...
        def do_GET(self) -> None:  # noqa: N802
            ...
            try:
                ...
                if path == "/api/v1/commercial_investment_committee_memos/latest":
                    ...
                    return
                if path == "/api/v1/commercial_saleability_gates/latest":
                    self._send(orchestrator.commercial_saleability_gate_report(
                        locale_bundles=ADMIN_TRANSLATIONS,
                        security_profile=security.readiness_profile(),
                    ))
                    return
                if path == "/api/v1/commercial_mutual_action_plans/latest":
                    self._send(orchestrator.commercial_mutual_action_plan_report(
                        locale_bundles=ADMIN_TRANSLATIONS,
                        security_profile=security.readiness_profile(),
                    ))
                    return
