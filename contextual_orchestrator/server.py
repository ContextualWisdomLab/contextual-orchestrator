...
def _validate_messages(messages: Any) -> list[dict[str, str]]:
    """Validate OpenAI chat messages including tool-role ``tool_call_id``.

    Tool messages must carry a non-empty ``tool_call_id`` string (≤128 chars).
    Non-tool roles must not send ``tool_call_id`` so clients cannot attach
    dangling call ids that the gateway would silently drop.
    """
    ...
    for message in messages:
        ...
        if role == "tool":
            ...
            raise RequestError(
                400,
                "invalid_tool_call_id",
                "tool_call_id is only valid on tool role messages",
            )
...
def _validate_chat_tool_choice(body: dict[str, Any]) -> str | dict[str, Any] | None:
    """OpenAI chat ``tool_choice`` — none/auto/required or function object.

    Requires ``tools`` to be present (enforced by the chat handler). When
    ``tool_choice`` names a function, that name must appear in ``tools`` so
    clients cannot request a tool the request body does not define.
    """
    ...
    if isinstance(choice, dict):
        ...
        if name not in tool_names:
            raise RequestError(
                400,
                "invalid_tool_choice",
                "tool_choice.function.name must match a tools function name",
            )
        return choice
    raise RequestError(
        400,
            ...
            try:
                ...
                if path == "/v1/chat/completions":
                    ...
                    if "store" in body:
                        ...
                        # Gateway does not persist chat for OpenAI store=true semantics; accept false, reject true.
                        if store is True:
                            raise RequestError(
                                400,
                                "invalid_store",
