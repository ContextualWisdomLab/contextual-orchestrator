...
class SqlLedgerStore:
    ...
    def _sql(self, statements: dict[str, str]) -> str:
        return statements.get(self._paramstyle, statements["pyformat"])
    ...
    def _seed_dimension_catalog(self) -> None:
        ...
        for order, (name, label, _column) in enumerate(ATTRIBUTION_DIMENSION_CATALOG):
            ...
            cur.execute(self._sql(_SQL_SELECT_DIMENSION), (name,))
            if cur.fetchone() is None:
                # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
                cur.execute(self._sql(_SQL_INSERT_DIMENSION), (name, label, order))
        self._conn.commit()
    ...
    def query(self, start: Optional[int] = None, end: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        if start is not None and end is not None:
            sql = self._sql(_SQL_SELECT_USAGE_WINDOW)
            params.extend((start, end))
        elif start is not None:
            sql = self._sql(_SQL_SELECT_USAGE_SINCE)
            params.append(start)
        elif end is not None:
            sql = self._sql(_SQL_SELECT_USAGE_UNTIL)
            params.append(end)
        else:
            sql = _SQL_SELECT_USAGE
        cur = self._conn.cursor()
        # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
