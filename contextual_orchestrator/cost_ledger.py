...
class SqlLedgerStore:
    ...
    def _seed_dimension_catalog(self) -> None:
        ...
        for order, (name, label, _column) in enumerate(ATTRIBUTION_DIMENSION_CATALOG):
            cur.execute(  # ph is a fixed DB-API placeholder ("?"/"%s"); the value is parameterized. nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
    ...
    def append(self, record: UsageRecord) -> None:
        ...
        cur = self._conn.cursor()
        cur.execute(  # columns/placeholders come from the fixed _USAGE_COLUMNS tuple; row values are parameterized. nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
    ...
    def query(self, start: Optional[int] = None, end: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        columns = ", ".join(_USAGE_COLUMNS)
        cur = self._conn.cursor()
        cur.execute(f"SELECT {columns} FROM llm_usage_records{where}", tuple(params))  # nosec B608 - columns and clauses are fixed strings with DB-API placeholders; values are parameterized. nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query
