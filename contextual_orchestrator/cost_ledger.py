...
class NoopUsageTelemetrySink:
    ...
    def emit_usage(self, event: UsageTelemetryEvent) -> None:
        """Discard the usage telemetry event (no sink is wired)."""
        return None
...
class InMemoryUsageTelemetrySink:
    ...
    def emit_usage(self, event: UsageTelemetryEvent) -> None:
        """Record one usage event, evicting oldest entries past the cap."""
        with self._lock:
    ...
    def events(self) -> List[UsageTelemetryEvent]:
        """Return a snapshot copy of the retained usage events."""
        with self._lock:
...
class UsageTelemetryHealth:
    ...
    def as_dict(self) -> Dict[str, Any]:
        """Return the health counters as a plain operator-safe mapping."""
        return {
...
class NonBlockingLedgerStore:
    ...
    def telemetry_health(self) -> Dict[str, Any]:
        """Return a thread-safe snapshot of the ledger export health counters."""
        with self._lock:
...
class SqlLedgerStore:
    ...
    def _seed_dimension_catalog(self) -> None:
        ...
        for order, (name, label, _column) in enumerate(ATTRIBUTION_DIMENSION_CATALOG):
            # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query -- {ph} is a DB-API placeholder; the value is bound, never interpolated.
            cur.execute(
    ...
    def append(self, record: UsageRecord) -> None:
        ...
        cur = self._conn.cursor()
        # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query -- columns/placeholders are the fixed _USAGE_COLUMNS constant and DB-API placeholders; values are bound.
        cur.execute(
    ...
    def query(self, start: Optional[int] = None, end: Optional[int] = None) -> List[Dict[str, Any]]:
        ...
        columns = ", ".join(_USAGE_COLUMNS)
        cur = self._conn.cursor()
        # nosemgrep: python.sqlalchemy.security.sqlalchemy-execute-raw-query.sqlalchemy-execute-raw-query -- columns/where are the fixed _USAGE_COLUMNS constant and DB-API placeholder clauses; params are bound.
