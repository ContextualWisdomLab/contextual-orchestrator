...
def exercise_reasoning_effort_profile(value: Any) -> None:
    ...
    parse is finite. ``true_theta`` is stripped before parse so Hypothesis
    and Atheris can reach the ablation. An ablation against that vector
    either fails closed or stays production-locked while
    ``measurement_status`` is estimated.
    """
    if not isinstance(value, dict):
        return
    payload = dict(value)
    theta = payload.pop("true_theta", None)
    try:
