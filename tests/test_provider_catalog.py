...
def test_fixed_inventory_covers_five_secrets_and_keeps_nvidia_accounts_distinct() -> None:
    ...
    assert DEFAULT_PROVIDER_ACCOUNTS[0].models_url.endswith("/v1/models")
    assert DEFAULT_PROVIDER_ACCOUNTS[2].models_url.endswith("/models/v2")


def test_required_bootstrap_registers_all_values_but_returns_names_only() -> None:
    """Trusted bootstrap writes the complete generation and never echoes a value."""
    environment = {
...
def test_required_bootstrap_validates_complete_inventory_before_any_write() -> None:
    """A missing production secret prevents a partially rotated database state."""
    environment = {
...
def test_refresh_isolates_provider_failure_and_preserves_last_known_good_models() -> None:
...
def test_refresh_fails_closed_when_only_non_chat_or_failed_models_exist() -> None:
...
def test_candidate_agents_filter_non_chat_models_and_keep_account_credentials_distinct() -> None:
...
def test_disabled_account_is_not_served_but_its_catalog_history_remains() -> None:
...
def test_bytez_native_chat_serializes_text_messages_and_normalizes_output() -> None:
    ...
    def bytez_request(agent, prompt, credential):
...
def test_bytez_failures_do_not_guess_credentials_messages_or_passthrough_shapes() -> None:
    ...
    with pytest.raises(ProviderCatalogUnavailable, match="text-only"):
        client.chat(agent, [{"role": "user", "content": ["not", "text"]}])
...
def test_schema_is_normalized_and_contains_no_provider_secret_value_column() -> None:
    """Catalog metadata references credential names and separates multivalued facts."""
