"""Chat Completions logprobs/top_logprobs fail-closed when provided (not applied)."""
...
_TEST_AUTH_TOKEN = "chat_logprobs_unsup_token"  # noqa: S105
...
def test_http_chat_rejects_non_boolean_logprobs() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
if __name__ == "__main__":
    ...
    test_http_chat_rejects_non_boolean_logprobs()
