"""Responses model: required non-empty string; missing/empty fail-closed."""
...
_TEST_AUTH_TOKEN = "responses_model_required_token"  # noqa: S105
...
def test_http_responses_accepts_model() -> None:
    server, thread, port = _server()
    try:
        status, body = _post(
            port,
            {"model": "mock-generalist", "input": "invoice total please"},
        )
...
def test_http_responses_rejects_empty_model() -> None:
    server, thread, port = _server()
    try:
        status, body = _post(port, {"model": "  ", "input": "x"})
        assert status == 400, body
        assert "invalid_model" in json.dumps(body)
    finally:
        server.shutdown()
...
def test_http_responses_rejects_non_string_model() -> None:
    server, thread, port = _server()
    try:
        status, body = _post(port, {"model": 12, "input": "x"})
        assert status == 400, body
        assert "invalid_model" in json.dumps(body)
    finally:
        server.shutdown()
...
if __name__ == "__main__":
    test_http_responses_accepts_model()
