"""Chat messages: role=tool requires non-empty tool_call_id (OpenAI shape)."""
...
def test_http_chat_rejects_tool_message_without_tool_call_id() -> None:
    ...
    try:
        ...
        assert "tool_call_id" in body["error"]["message"]
    finally:
        server.shutdown()
        thread.join(timeout=5)


def test_http_chat_rejects_empty_tool_call_id() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
def test_http_chat_rejects_tool_call_id_too_long_on_passthrough() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
    try:
        ...
        assert status == 400, body
        assert body["error"]["code"] == "invalid_message"
        assert "128" in body["error"]["message"]
    finally:
        server.shutdown()
        thread.join(timeout=5)
...
if __name__ == "__main__":
    ...
    test_http_chat_rejects_empty_tool_call_id()
    test_http_chat_rejects_tool_call_id_too_long_on_passthrough()
