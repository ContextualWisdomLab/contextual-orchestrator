...
@pytest.mark.parametrize("candidate_count", [9, 401, 403])
def test_selective_coverage_uses_actual_odd_sized_strata(monkeypatch, candidate_count):
    ...
    def oracle(_bundle, responses, **_kwargs):
        ...
        if not responses:
            candidate_index = (candidate_index + 1) % candidate_count
        theta = -2.0 + 4.0 * (candidate_index + 0.5) / candidate_count
        return {
    ...
    for point in (screen["heldout"], screen["heldout_baseline"]):
        for metric in ("coverage", "near_cut_coverage", "negative_coverage", "positive_coverage"):
...
def test_calibration_rejects_undefined_resolution_summary(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda _bundle, responses, **_kwargs: {
            "ranked_items": [len(responses)], "theta_eap": [0.0], "theta_sd": [1.0]
        },
    )
...
def test_observation_p95_tracks_actual_sample_count(monkeypatch, capsys, sample_count):
    ...
    elapsed_ns = [1_000_000] * 9 + [index * 1_000_000 for index in range(1, sample_count + 1)]
...
def test_heldout_runtime_guard_precedes_optional_dependency_imports(monkeypatch):
    ...
    with pytest.raises(SystemExit, match="uv run --python 3.12 python scripts/benchmark_psychometric_heldout.py"):
