...
def test_zen_only_account_keeps_zen_models_when_shared_go_source_fails(
    ...
    try:
        ...
        assert report.selected_agent_ids == (agent_id_for(_model(zen, "zen-live")),)
    finally:
        set_backend(None)
...
def test_shared_credential_rolls_back_only_when_every_sharing_source_fails() -> None:
    ...
    try:
        ...
        assert get_credential("OPENCODE_ZEN_API_KEY") == "zen-subscription-key"
        assert agent_id_for(_model(go, "go-live")) in zen_down.selected_agent_ids
...
def test_inventory_verdict_rejects_nontransient_shared_source_failure() -> None:
    """A transient sibling cannot excuse a shared source's auth failure."""
    zen = _source("opencode_zen", "OPENCODE_ZEN_API_KEY")
    go = _source("opencode_go", "OPENCODE_ZEN_API_KEY")
    verdict = evaluate_provider_credential_inventory(
        {
            "registered_credentials": [],
            "restored_credentials": ["OPENCODE_ZEN_API_KEY"],
            "providers_with_errors": ["opencode_zen", "opencode_go"],
            "provider_error_classifications": {
                "opencode_zen": TRANSIENT_FAILURE_CLASSIFICATION,
                "opencode_go": TRANSIENT_FAILURE_CLASSIFICATION,
            },
            "provider_account_error_classifications": {
                "opencode_go": {
                    "OPENCODE_ZEN_API_KEY": AUTHENTICATION_FAILURE_CLASSIFICATION
                }
            },
        },
    ...
    assert verdict.ok is False
    assert AUTHENTICATION_FAILURE_CLASSIFICATION in verdict.hard_fail_reason
