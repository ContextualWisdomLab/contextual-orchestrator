"""Chat Completions logprobs/top_logprobs honesty over HTTP (route never emits them)."""
...
_TEST_AUTH_TOKEN = "chat_logprobs_top_logprobs_http_honesty_token"  # noqa: S105
...
def test_http_completions_rejects_token_logprobs() -> None:
    """Legacy Completions integer logprobs are fail-closed (response always null)."""
    server, thread, port = _server()
    try:
        status, body = _post(
            port,
...
if __name__ == "__main__":
    ...
    test_http_completions_rejects_token_logprobs()
