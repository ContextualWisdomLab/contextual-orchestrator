...
not verify its certificate chain (custom CA not in Python's trust store). A per-client
CA bundle now solves that without an insecure verification bypass.
"""
...
def test_insecure_skip_verify_is_rejected() -> None:
    try:
        ...
        raise AssertionError("provider TLS verification bypass must be rejected")
