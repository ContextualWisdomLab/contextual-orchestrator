...
def test_build_review_orchestrator_accepts_text_capable_multimodal_model(monkeypatch):
    """Supported image input does not mean every request requires an image."""
    discovered = [
        DiscoveredModel(
    ...
    orchestrator = review_gateway.build_review_orchestrator(
        {"NVIDIA_NIM_API_KEY": "nim-secret"}, max_agents=1
    )
