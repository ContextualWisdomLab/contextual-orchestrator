"""`--log-level`/`--verbose`/`--debug` CLI wiring."""
...
_REMOVED_ENV_VAR = "CONTEXTUAL_ORCHESTRATOR_LOG_LEVEL"
...
def test_default_level_is_warning_without_any_flag() -> None:
...
def test_log_level_environment_variable_is_not_runtime_configuration() -> None:
    with _restored_root_logger():
        os.environ[_REMOVED_ENV_VAR] = "DEBUG"
        try:
            _run_one_shot([])
        finally:
            del os.environ[_REMOVED_ENV_VAR]
        assert logging.getLogger().getEffectiveLevel() == logging.WARNING
...
if __name__ == "__main__":  # pragma: no cover
    ...
    test_default_level_is_warning_without_any_flag()
    ...
    test_log_level_environment_variable_is_not_runtime_configuration()
