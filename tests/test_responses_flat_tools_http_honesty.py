"""Responses flat tools shape (type/name/parameters) over HTTP."""
...
_TEST_AUTH_TOKEN = "responses_flat_tools_http_honesty_token"  # noqa: S105
...
def test_http_chat_rejects_flat_function_tools_without_single_agent_fallback() -> None:
    ...
    try:
        status, body = _post(
            ...
            {
                "model": "mock-planner",
                "messages": [{"role": "user", "content": "flat on chat"}],
                "tools": [
                    {
                        "type": "function",
                        "name": "lookup_item",
...
def test_http_tools_rejects_mixed_nested_and_flat() -> None:
    server, thread, port = _server()
    try:
        status, body = _post(
            port,
...
if __name__ == "__main__":
    test_http_responses_rejects_flat_function_tools_without_single_agent_fallback()
    test_http_responses_rejects_flat_tools_with_tool_choice_name()
    test_http_chat_rejects_nested_function_tools_without_single_agent_fallback()
    test_http_chat_rejects_flat_function_tools_without_single_agent_fallback()
