"""Completions top_p is applied to ModelClient; out-of-range fail-closed."""
...
_TEST_AUTH_TOKEN = "cmpl_top_p_applied_token"  # noqa: S105
...
def test_http_completions_applies_top_p_to_client() -> None:
    orch = build()
    applied: list[float] = []
    client = orch.client
    ...
    try:
        status, body = _post(
            port,
            {"model": "mock-generalist", "prompt": "Hello", "top_p": 0.7},
        )
        assert status == 200, body
        assert 0.7 in applied, applied
...
def test_http_completions_accepts_top_p_one() -> None:
    ...
    try:
        status, body = _post(
            port,
            {"model": "mock-generalist", "prompt": "Hello", "top_p": 1},
        )
        assert status == 200, body
    finally:
        server.shutdown()
...
def test_http_completions_rejects_top_p_above_one() -> None:
    ...
    try:
        status, body = _post(
            port,
            {"model": "mock-generalist", "prompt": "Hello", "top_p": 1.1},
        )
        assert status == 400, body
        assert "invalid_top_p" in json.dumps(body)
    finally:
...
if __name__ == "__main__":
    test_http_completions_applies_top_p_to_client()
    test_http_completions_accepts_top_p_one()
