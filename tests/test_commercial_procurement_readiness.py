...
def test_commercial_procurement_blocked_rule_names_document_mismatch() -> None:
    orchestrator = build()
    report = orchestrator.commercial_procurement_readiness_report(target_contract_value_krw=TARGET_CONTRACT_VALUE_KRW)
    rules = {row["procurement_status"]: row["rule"] for row in report["procurement_status_rules"]}
    assert "document mismatch" in rules["commercial_procurement_blocked"]
...
if __name__ == "__main__":  # pragma: no cover
    ...
    test_commercial_procurement_blocked_rule_names_document_mismatch()
    print("ok")
