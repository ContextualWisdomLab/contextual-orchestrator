...
def test_local_embedding_backend_counts_tokens_without_counter() -> None:
    # No token_counter injected -> word-split fallback for the token count.
...
class _FakeEmbeddingApiClient:
    ...
    async def download_results(self, batch_id, endpoint_alias):
        ...
        # Deliberately out of order: index mapping must restore submission order.
        return {
            "success": True,
            "responses": [
...
def test_pg_llm_batch_embedding_backend_submits_polls_and_orders_results() -> None:
    ...
    # Restored to submission order (a before b) despite the reversed download.
...
def test_emit_usage_event_swallows_sink_failures() -> None:
    ...
    # Must not propagate: telemetry export is best-effort.
...
def test_non_blocking_store_drops_on_full_queue_and_flush_reports_timeout_then_stores() -> None:
    ...
    assert backend.started.wait(timeout=5)  # worker is now blocked holding "first"
    store.append(_record("queued"))  # fills the single queue slot
    store.append(_record("dropped"))  # queue full -> dropped

    # Flush cannot complete while the worker is blocked.
...
def test_cost_ledger_inline_store_failure_is_recorded_and_survives() -> None:
    ...
    # Only the exception type is exported, never the message text.
...
def test_sql_ledger_store_query_honours_time_window() -> None:
    ...
    assert len(store.query(150, 400)) == 1  # start + end
    assert len(store.query(None, 200)) == 1  # end only
    assert len(store.query(200, None)) == 1  # start only
    assert len(store.query()) == 2  # no window
...
def test_weighted_average_embedding_empty_and_weighted() -> None:
    ...
    # Weighted mean of two aligned vectors by their token weights.
...
def test_force_token_safe_chunks_empty_and_single_unit_midpoint_split() -> None:
    coordinator = _coordinator()
    # An empty original input short-circuits to a single empty part.
    ...
    # A single oversized no-whitespace unit under the char cap is split at the midpoint.
...
def test_count_embedding_tokens_tolerates_counter_failure_and_zero() -> None:
    coordinator = _coordinator()
    # Whitespace text counts as zero heuristically, but non-empty text floors at 1.
    ...
    assert failing._count_embedding_tokens("hi there", "m") == 2  # word-split fallback
...
def test_embeddings_batch_document_returns_pending_envelope_when_incomplete() -> None:
    ...
    # No usage recorded for a still-pending batch.
...
def test_embeddings_batch_document_handles_missing_source_parts_and_zero_token_items() -> None:
    class _PartialBackend:
        ...
        def retrieve(self, job):
            # Only return a result for the first input (source 0); source 1 gets none,
            # and report zero prompt_tokens so the request-token fallback is used.
    ...
    # Source 1 had no returned parts -> empty embedding, zero tokens.
    assert document["embeddings"][1]["embedding"] == []
    assert document["token_counts"][1] == 0
    # Source 0 recovered its token count from the request even though the item reported zero.
