...
def test_local_embedding_backend_token_fallback_counts_words() -> None:
    ...
    backend = LocalEmbeddingBatchBackend(dimension=4)
    job = backend.submit(
...
class _FakeEmbeddingClient:
    ...
    def __init__(self) -> None:
        """Start with an empty backend-call log."""
        self.calls: list[str] = []
...
def test_pg_embedding_backend_incomplete_download_returns_empty() -> None:
    ...
    class _IncompleteClient(_FakeEmbeddingClient):
        """Fake client whose result download is explicitly unsuccessful."""
...
class _FakeAssembler:
    ...
    def __init__(self) -> None:
        """Start without an assembled payload."""
        self.assembled = None
