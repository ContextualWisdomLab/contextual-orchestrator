"""Completions logit_bias: empty object accepted as no-op; non-empty fail-closed."""
...
_TEST_AUTH_TOKEN = "cmpl_logit_bias_empty_token"  # noqa: S105
...
def test_http_completions_omits_logit_bias_ok() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
def test_http_completions_rejects_logit_bias_non_object() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
if __name__ == "__main__":
    ...
    test_http_completions_omits_logit_bias_ok()
    test_http_completions_rejects_nonempty_logit_bias()
    test_http_completions_rejects_logit_bias_non_object()
    print("ok")
