"""Chat Completions modalities: ["text"] ok; audio or other values fail closed."""
...
_TEST_AUTH_TOKEN = "chat_modalities_token"  # noqa: S105
...
def test_http_chat_rejects_modalities_with_audio() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
if __name__ == "__main__":
    ...
    test_http_chat_rejects_modalities_with_audio()
