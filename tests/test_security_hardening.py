...
def test_external_bearer_admin_reads_require_object_policy() -> None:
    orchestrator = build()
    run_id = orchestrator.run([{"role": "user", "content": "hello"}], mode="route")["workflow_run_id"]
    ...
    server = build_server(
        orchestrator,
        port=0,
        security=SecurityConfig(bearer_verifier=verify, resource_authorizer=deny),
    )
    ...
    assert seen == [("admin", "workflow_run", run_id, "127.0.0.1")]


def test_external_bearer_admin_reads_without_object_policy_fail_closed() -> None:
    def verify(token: str, scope: str) -> bool:
        return token == "keyverse-admin" and scope == "admin"
    ...
    try:
        status, body = get_json(
            f"http://127.0.0.1:{port}/api/v1/workflow_runs/run_unknown",
            token="keyverse-admin",
        )
    finally:
        server.shutdown()
    ...
    assert status == 403
    assert body["error"]["code"] == "resource_authorization_required"
