"""Chat Completions response_format must be a typed OpenAI object."""
...
_TEST_AUTH_TOKEN = "chat_response_format_token"  # noqa: S105
...
def test_http_chat_rejects_json_schema_without_name() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
def test_http_chat_accepts_json_object_response_format_shape() -> None:
    server = build_server(build(), port=0, security=SecurityConfig(auth_token=_TEST_AUTH_TOKEN))
    thread = threading.Thread(target=server.serve_forever, daemon=True)
    thread.start()
    port = server.server_address[1]
...
if __name__ == "__main__":
    ...
    test_http_chat_rejects_json_schema_without_name()
    test_http_chat_accepts_json_object_response_format_shape()
