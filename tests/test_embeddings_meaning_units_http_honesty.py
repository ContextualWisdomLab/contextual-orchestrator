...
def test_http_short_last_line_keeps_leftover_base64_out_of_the_invoice() -> None:
    server, thread, port = _serve()
    try:
        status, body = _post(
            port,
            {
                "model": "mock-a",
                "inputs": [INVOICE_IMAGE_SHORT_LAST_LINE_4],
                "chunking_strategy": "meaning_units",
            },
        )
        assert status == 200, body
        ...
        assert "YII=" not in invoice.chunk_text
        assert "data:image" not in invoice.chunk_text
    finally:
        server.shutdown()
        thread.join(timeout=5)
...
def test_http_source_document_strategy_is_omit() -> None:
    server, thread, port = _serve()
    try:
        status, body = _post(
            port,
...
if __name__ == "__main__":
    ...
    test_http_short_last_line_keeps_leftover_base64_out_of_the_invoice()
    test_http_source_document_strategy_is_omit()
