...
def test_default_openrouter_direct_discovery_remains_evidence_only() -> None:
    register_credential("OPENROUTER_API_KEY", "sk-router")
    source = next(
    ...
    with patch(
        "contextual_orchestrator.model_discovery.urllib.request.urlopen",
        return_value=_Response({"data": [{"id": "provider/unattested"}]}),
    ):
        discovered = discover_provider_models(source)

    assert len(discovered) == 1
    assert discovered[0].evidence_only is True
    assert is_routable_discovered_model(discovered[0]) is False
...
def test_discover_all_models_blocks_only_paid_openrouter_without_credit(
    ...
    with patch(
        ...
        side_effect=lambda request, timeout=None: _Response(
            payload
            if request.full_url == OPENROUTER_SOURCE.list_url
            else {
                "data": [
        ...
        return_value={"provider/free", "provider/paid"},
    ):
