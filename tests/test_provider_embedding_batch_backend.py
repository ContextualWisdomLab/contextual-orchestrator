...
def test_default_client_keeps_batch_lifecycle_separate_from_model_timeout() -> None:
    """A null model timeout leaves execution unbounded and registry claims separate."""
...
def test_durable_provider_embedding_backend_survives_unbounded_client_timeout() -> None:
    ...
    assert backend._claim_lease_seconds == cost_router_module._DEFAULT_PROVIDER_EMBEDDING_CLAIM_LEASE_SECONDS
    backend.close()
