...
def load_contract() -> dict:
    ...
    return json.loads(
        Path("tests/fixtures/review_inference_preflight_v1.json").read_text()
    )
...
def serve_gateway(*, free_available=True, zdr_available=True, reject_calls=False):
    ...
    agents = [
        ModelAgent("paid_agent", "paid-model", base_url="https://paid.invalid/v1")
    ]
    if free_available:
        agents.insert(
            0,
    ...
    server = build_server(
        ...
        security=SecurityConfig(
            admin_token="fixture_admin", inference_token="fixture_inference"
        ),
    )
...
def request_gateway(
    port_number, request_path, request_body=None, auth_token="fixture_inference"
):
...
def test_preflight_discovery_needs_only_inference_scope():
    ...
    with serve_gateway() as (port_number, _client):
        for auth_token in (None, "wrong_fixture_token"):
            assert (
                request_gateway(
                    port_number, contract["discovery_path"], auth_token=auth_token
                )[0]
                == 401
            )
        status_code, response_body = request_gateway(
            port_number, contract["discovery_path"]
        )
...
def test_inference_preflight_preserves_capability_requests(probe_name):
    ...
    with serve_gateway() as (port_number, client):
        status_code, response_body = request_gateway(
            port_number, contract["inference_path"], probe_request
        )
...
def test_preflight_exhaustion_never_reaches_paid_or_non_zdr_routes(gateway_options):
    ...
    with serve_gateway(**gateway_options) as (port_number, client):
        status_code, response_body = request_gateway(
            port_number,
            contract["inference_path"],
            contract["probe_requests"]["json_object"],
        )
        assert status_code >= 400, response_body
        assert "paid_agent" not in {
            agent_id for agent_id, _payload in client.provider_calls
        }
