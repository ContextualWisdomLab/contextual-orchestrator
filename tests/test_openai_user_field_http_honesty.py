...
def test_http_completions_accepts_user() -> None:
    ...
    try:
        status, body = _post(
            port,
            "/v1/completions",
            {"model": "mock-generalist", "prompt": "hi", "user": "buyer_account_9"},
        )
...
def test_http_completions_rejects_empty_user() -> None:
    ...
    try:
        status, body = _post(
            port,
            "/v1/completions",
            {"model": "mock-generalist", "prompt": "hi", "user": ""},
        )
...
def test_http_completions_rejects_null_user() -> None:
    ...
    try:
        status, body = _post(
            port,
            "/v1/completions",
            {"model": "mock-generalist", "prompt": "hi", "user": None},
        )
        assert status == 400, body
        assert "invalid_user" in json.dumps(body)
    finally:
...
def test_http_embeddings_rejects_empty_user() -> None:
    ...
    try:
        status, body = _post(
            port,
            "/v1/embeddings",
            {"model": "text-embedding-test", "input": "x", "user": ""},
        )
        assert status == 400, body
        assert "invalid_user" in json.dumps(body)
    finally:
