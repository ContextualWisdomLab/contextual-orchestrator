...
- `/v1/chat/completions` accepts normal chat messages, and `"stream": true` returns an OpenAI-compatible `text/event-stream` of `chat.completion.chunk` deltas terminated by `data: [DONE]`. In **route** mode the worker's tokens are streamed live as they arrive from the provider (real token streaming); in **conduct** mode the multi-step answer is produced then framed as deltas (a workflow can't honestly token-stream a synthesizer that hasn't run yet). SDK-default `tools[].function.strict: null` and `response_format.json_schema.strict: null` are omit-equivalent and stripped before passthrough. On `/v1/responses`, send official `{type, name, parameters, strict}` function tools (chat-shaped tools still work) and set `parallel_tool_calls=true` only with a non-empty `tools` array. Chat rejects the Responses-native shape. `"stream": true` on `/v1/responses` returns official `response.created` / `response.output_text.delta` / `response.completed` SSE frames whose concatenated deltas equal the non-stream `output_text` (framed from the completed passthrough, not live provider token SSE).
