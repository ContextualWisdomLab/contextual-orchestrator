...
- `/v1/chat/completions` accepts normal chat messages, and `"stream": true` returns an OpenAI-compatible `text/event-stream` of `chat.completion.chunk` deltas terminated by `data: [DONE]`. In **route** mode the worker's tokens are streamed live as they arrive from the provider (real token streaming); in **conduct** mode the multi-step answer is produced then framed as deltas (a workflow can't honestly token-stream a synthesizer that hasn't run yet). Unknown `stream_options` keys fail closed with `invalid_stream_options` even when the value is null, including on the tools / `response_format` passthrough path. Allowed `include_usage` / `include_obfuscation` nulls stay omit-equivalent; `true` is unsupported here — drop those flags or keep them false.
