...
- `/v1/chat/completions` accepts normal chat messages, and `"stream": true` returns an OpenAI-compatible `text/event-stream` of `chat.completion.chunk` deltas terminated by `data: [DONE]`. `stream_options.include_usage=true` is accepted on every chat stream, including structured `tools`/`response_format` passthrough, and emits a usage-only chunk after the terminal stop chunk: real provider-reported usage when the completed response carries it (`usage_source: "reported"`), otherwise an honestly labeled estimate (`usage_source: "estimated"`) — never silently omitted or fabricated as reported. In **route** mode the worker's tokens are streamed live as they arrive from the provider (real token streaming); in **conduct** mode and structured/tools passthrough, the complete answer is produced first, then framed as deltas (a workflow can't honestly token-stream a synthesizer that hasn't run yet, and tool-call arguments arrive as one complete provider response).
