...
- Verbose/debug logging is off by default. Pass `--verbose`/`--debug` (or set `CONTEXTUAL_ORCHESTRATOR_VERBOSE=true` and restart, for an already-deployed server — this is a one-shot startup flag, not a live/dynamic toggle) to enable stdlib DEBUG-level logging of the route-vs-conduct decision, each route/conduct step's agent and latency, provider request/retry/backoff attempts, circuit-breaker open/half-open/closed transitions, and model discovery. DEBUG is enabled only on these specific, individually audited loggers — never the root logger — so an unrelated, unaudited `.debug()` call site elsewhere in the process cannot become newly visible. Log records are bounded and secret-free by construction: never an API key, an `Authorization` header, or prompt/response content — see [ADR 0125](docs/planning/adrs/0125-verbose-debug-logging.md).
