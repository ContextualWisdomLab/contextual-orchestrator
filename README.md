...
- Verbose/debug logging is off by default. Pass `--verbose`/`--debug` (or set `CONTEXTUAL_ORCHESTRATOR_VERBOSE=true` and restart, for an already-deployed server) to enable stdlib DEBUG-level logging of the route-vs-conduct decision, each route/conduct step's agent and latency, provider request/retry/backoff attempts, and circuit-breaker open/half-open/closed transitions. Log records are bounded and secret-free by construction: never an API key, an `Authorization` header, or prompt/response content — see [ADR 0125](docs/planning/adrs/0125-verbose-debug-logging.md).
