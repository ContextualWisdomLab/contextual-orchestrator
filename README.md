...
- Model-group routing observations remain process-local unless an operator explicitly passes `--routing-observation-window-seconds SECONDS` with `--state-db PATH`. That opt-in SQLite store shares only completed success/failure and measured latency/token observations inside the selected time window; it applies no decay or cross-model quality weighting. Non-stream persistence errors fail closed; if a stream has already emitted provider bytes, a write failure is logged and cannot change the completed response.
