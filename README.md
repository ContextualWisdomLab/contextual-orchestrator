...
- `/v1/chat/completions` accepts normal chat messages, and `"stream": true` returns an OpenAI-compatible `text/event-stream` of `chat.completion.chunk` deltas terminated by `data: [DONE]` on the orchestration path. In **route** mode the worker's tokens are streamed live as they arrive from the provider (real token streaming); in **conduct** mode the multi-step answer is produced then framed as deltas (a workflow can't honestly token-stream a synthesizer that hasn't run yet). When the body includes `tools` or `response_format`, send a named pool `model` and omit `stream` (or send `stream=false`): that path proxies one agent and has no SSE plane, so omitted model and `stream=true` return named 400s instead of a silent worker or a JSON body.
