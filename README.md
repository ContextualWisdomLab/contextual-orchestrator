...
Model-based conduct verification requires `fast-mlsirm` in the same runtime and fails closed when it is absent or broken; fast-mlsirm sends its judge completion through this contextual-orchestrator gateway, so no direct provider fallback is used. Before a live judge benchmark, run `python -m contextual_orchestrator check-fast-mlsirm` with the exact interpreter that will run fast-mlsirm. It prints the interpreter, package version, transitive-import status, and contextual contract check, and exits nonzero on a missing dependency or contract mismatch. See [ADR 0001](docs/planning/adrs/0001-fail-closed-model-judgment.md).
