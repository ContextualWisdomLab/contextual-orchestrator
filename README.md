...
- `/api/v1/provider_readiness/latest` and `/v1/provider_readiness` report provider liveness separately from an explicit chat readiness probe. Before the first refresh they return `unprobed`; `?refresh=true` performs one cancellable probe with a bounded wall-clock deadline and seeds the latest cached result for later reads on both routes.
