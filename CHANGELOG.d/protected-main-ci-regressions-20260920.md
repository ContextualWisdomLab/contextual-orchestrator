...
- Psychometric observation no longer holds `_psychometric_persistence_lock` across the embedding call: the vector depends only on the prompt context, so it is computed first and the lock now covers only the deployment revalidation and persistence. The served deployment is revalidated under the lock after the embedding, so a pool change during that provider-latency window drops the observation instead of filing it against a stale deployment revision, and a concurrent `patch_agent` no longer waits behind it.
- Direct-route judging now carries the selected deployment identity into observation. An agent edit during judging drops the old answer instead of recording it against the new deployment.
