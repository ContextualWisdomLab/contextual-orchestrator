# Graph Report - token-scope-corrected-evidence  (2026-09-13)

## Corpus Check
- cluster-only mode — file stats not available

## Summary
- 412 nodes · 1033 edges · 26 communities (24 shown, 2 thin omitted)
- Extraction: 99% EXTRACTED · 1% INFERRED · 0% AMBIGUOUS · INFERRED: 11 edges (avg confidence: 0.86)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `3345a313`
...
- .principal_id
- test_completions_max_tokens_http_honesty.py
...
### Community 0 - "build_server"
Cohesion: 0.09
Nodes (47): build_server(), _admin_purpose(), _audit_trace_disclosure(), _authorize(), _authorize_trace_access(), _begin_sse(), _write(), _bind_session() (+39 more)

### Community 1 - "_coerce_optional_bool"
Cohesion: 0.06
Nodes (32): _coerce_optional_bool(), Legacy Completions ``echo`` — boolean / JS 0/1; ``true`` is not supported.…, Responses ``logprobs`` / ``top_logprobs`` — OpenAI shape; invalid fail closed.…, Responses ``parallel_tool_calls`` — strict boolean when present. OpenAI uses…, Legacy Completions ``logprobs`` — token logprobs are not supported. This…, Legacy Completions ``stream_options`` — object with boolean flags; requires…, Chat Completions ``stream_options`` — validate supported streaming flags. Shape…, Fail-closed chat ``logprobs`` / ``top_logprobs`` before any proxy. Chat route… (+24 more)

### Community 2 - "_validate_chat_sampling_and_control_fields"
Cohesion: 0.07
...
### Community 3 - "do_POST"
Cohesion: 0.07
Nodes (29): _cache_partition(), do_POST(), _validate_trace_request(), _chat_response_sse_chunks(), _chat_usage_measurement_payload(), _multipart_upload_metadata(), Read purpose, filename, and file length from a seekable multipart body., Validate or default the Chat Completions model. Chat exposes the advertised… (+21 more)

### Community 4 - "server.py"
Cohesion: 0.09
Nodes (23): _cache_bypass_header(), _orchestrated_response(), HTTP server exposing chat, admin, governance, and evaluation endpoints., Validate or default the chat/completions model. An omitted ``model`` selects…, OpenAI ``metadata`` — object of string pairs, at most 16 entries. Keys must be…, Responses ``modalities`` — omit or ``["text"]`` only (text gateway)., Responses API ``instructions`` — optional non-empty string ≤32000 chars. OpenAI…, Validate or auto-select an OpenAI embeddings model. Strip + write back (parity… (+15 more)

### Community 5 - "RequestError"
Cohesion: 0.09
Nodes (23): _read_json(), _coerce_json(), Responses ``stop`` — string or ≤4 non-empty strings (≤256 chars); pass through.…, Legacy Completions ``suffix`` — optional string; non-empty is not supported.…, Responses ``stream_options`` — not supported (Responses streaming is off).…, Fail closed on role=tool messages missing a usable tool_call_id. Runs before…, OpenAI assistant ``tool_calls`` array shape on chat messages. Each entry must…, HTTP-safe request failure. (+15 more)

### Community 6 - "_coerce_optional_int"
Cohesion: 0.09
...
### Community 7 - "Any"
Cohesion: 0.16
Nodes (18): Any, BatchRequest, _embeddings_attribution(), _encode_embedding_base64(), _openai_embeddings_response(), OpenAI multimodal content-parts array (text + image_url) for vision callers.…, Fail closed on chat message keys outside the OpenAI surface we honor. Named…, Reject unknown message keys and legacy function role before passthrough. (+10 more)
...
### Community 10 - "_non_omit_object_entries"
Cohesion: 0.12
...
### Community 11 - "test_chat_max_completion_tokens_http_honesty.py"
Cohesion: 0.31
...
### Community 17 - "test_completions_max_tokens_http_honesty.py"
Cohesion: 0.44
...
### Community 18 - "_validate_tool_function_fields"
Cohesion: 0.25
...
### Community 19 - "ResponsiveThreadingHTTPServer"
Cohesion: 0.39
...
### Community 21 - ".establish_admin_session"
Cohesion: 0.33
...
### Community 23 - "_validate_chat_tool_choice"
Cohesion: 0.50
...
- **Why does `build_server()` connect `build_server` to `do_POST`, `server.py`, `RequestError`, `Any`, `SecurityConfig`, `test_chat_max_completion_tokens_http_honesty.py`, `test_responses_max_output_tokens_http_honesty.py`, `test_responses_max_tokens_http_honesty.py`, `test_completions_max_completion_tokens_http_honesty.py`, `test_completions_max_tokens_http_honesty.py`, `ResponsiveThreadingHTTPServer`, `test_http_preserves_caller_generation_limit`?**
  _High betweenness centrality (0.178) - this node is a cross-community bridge._
- **Why does `RequestError` connect `RequestError` to `build_server`, `_coerce_optional_bool`, `_validate_chat_sampling_and_control_fields`, `do_POST`, `server.py`, `_coerce_optional_int`, `Any`, `SecurityConfig`, `_validate_embeddings_inputs`, `_non_omit_object_entries`, `_validate_responses_conversation_controls`, `.principal_id`, `_validate_tool_function_fields`, `.establish_admin_session`, `.authorize`, `_validate_chat_tool_choice`, `_validate_responses_reasoning`?**
  _High betweenness centrality (0.133) - this node is a cross-community bridge._
- **Why does `SecurityConfig` connect `SecurityConfig` to `build_server`, `server.py`, `test_chat_max_completion_tokens_http_honesty.py`, `test_responses_max_output_tokens_http_honesty.py`, `test_responses_max_tokens_http_honesty.py`, `test_completions_max_completion_tokens_http_honesty.py`, `.principal_id`, `test_completions_max_tokens_http_honesty.py`, `test_http_preserves_caller_generation_limit`, `.establish_admin_session`, `.authorize`, `.admin_session_clear_cookie_header`?**
  _High betweenness centrality (0.124) - this node is a cross-community bridge._
- **Should `build_server` be split into smaller, more focused modules?**
