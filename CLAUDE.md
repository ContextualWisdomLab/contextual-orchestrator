...
- Tool-bearing chat requests never enter the batch contract. Generated planning and non-worker conduct roles suppress caller tools, while worker calls keep them. Grouped structured synthesis records each failed member once before failover. Live Bytez catalog evidence from 2026-09-09 remains degraded: `task=chat` returned zero rows and the other tested list shapes returned HTTP 500.
