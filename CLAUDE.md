...
Agent pools are **data, not code**: `examples/agents.mock.json` and `examples/agents.openai.json`. State is in-memory by default; `--state-db PATH` or `process_bootstrap.state_database_path` persists runs/audit/analytics to sqlite.
