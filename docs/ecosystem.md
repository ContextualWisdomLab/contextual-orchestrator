...
## free-router pairing note

`free-router` is a **discovery CLI** for free/open model catalogs (e.g. NIM /
OpenRouter). Operators promote selected model IDs into this gateway's agent pool
JSON (and optional `model_group` replicas). Pairing is configuration, not a
runtime hard dependency.

Branch policy on free-router: PRs into that repo's `main` must come from
`release/*` or `hotfix/*` branches. Gateway pairing documentation and agent-pool
examples live **here** so product docs are not blocked on free-router merge
policy.
