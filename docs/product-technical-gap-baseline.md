...
| [#851](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/851) | `f1dae69d065df9915061174f979d28920e856773` | Current evidence head; virtual `auto` and `free` selectors retain provider failover, the free selector cannot cross into paid members, and missing image capability returns a client error. The hash-locked exact-head full suite passed `2295` tests with `0 failed` in `565.94s` (exit `0`); hosted Checks and independent approval remain required. |
