...
| [#807](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/807) | `8bdbd5f16e158aefbdf872c2824035da7a125a74` | open, based on main; 22 hosted check-runs queued at snapshot, approvals `0` | Provider error-boundary repair. The valid ProviderResponseError probe-classification finding was fixed on this exact head; local focused reliability/discovery/MLX evidence is `75 passed`, plus compileall and diff-check. Protected hosted Checks and independent approval remain required. Decision: `WAIT_AND_REMEDIATE`. |
...
| [#805](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/805) | `9a3f32d97b42d88ce5bb0a0dc2f100b5590e5547` | open, based on `fix/auto-reasoning-effort-contract-rebased` at `f1b0cd48271e870571b022463e1ec2c857ae4a8a`; mergeable clean, hosted check-runs absent, approvals `0` | Structured provider-feature orchestration and per-provider cost attribution repair. Current exact-head focused regression set is `172 passed`; compileall and diff-check pass. The prior parent `68bdda6` full-suite `1617 passed` and coverage (`93.16%` statement, `87.40%` branch) are explicitly stale after the current refactor. Devin's current findings are informational and dispositioned; independent approval and hosted required Checks remain required. Decision: `WAIT_AND_REMEDIATE`. |
