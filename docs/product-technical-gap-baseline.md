...
## 2026-08-26 19:51 KST protected-main and moving-queue evidence
...
| --- | --- | --- |
| [#849](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/849) | `566b74e59db0c97a842af2d8058dd220fda2ab4c` / `main` | The moving head adds one explicit connection-close header after the prior async-server work. GitHub reports `BLOCKED`; fifteen hosted jobs are pending and one informational CLI-validation review thread remains unresolved. The `229/229` k6 result (`128/128` inference and `101/101` liveness, zero request errors), official-runner diagnostic `2269 passed, 3 failed`, and alternate dependency-complete diagnostic `2272 passed` were measured locally on unpushed exact tree `b6703e4c4d16c01ba79e5e2fd6d1354c585eb24e`; they have no hosted workflow run ID and do not transfer to the current remote head or protected `main`. Acceptance is resolution of the current finding, a green official exact-head run, fresh k6 evidence, and counted review. |
| [#851](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/851) | `d49846d5c8deca55c60315a8fbb461ecdebf67eb` / `main` | The moving head closes issue #846's two remaining implementation gaps: generated workflows stop before the next provider call when the shared in-flight budget is spent, and omitted local-chat `max_tokens` uses the configured cap without overriding explicit input or mutating the request. It also preserves top-level measured provider usage for an empty trace. Independent exact-head reproduction passed `60` focused cost/generated-workflow/local-MLX tests; the owner records `67` focused and official `2352 passed, 0 failed`. These are PR-head evidence only. Acceptance remains terminal exact-head hosted gates, counted review, and normal protected-main integration; issue #846 stays open until then. |
| [#857](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/857) | `9ecf696803fa474cb0c2ad66c2acd4f854b05c56` / `main` | The moving head includes #867's request-scoped structured-provider reliability changes, the deployment-scoped embedding deduplication repair, and the normally merged #875 durable-readiness evidence repair. Sixteen hosted jobs are pending, no review threads are unresolved, and review is required. The complete `2366 passed` local result belongs to predecessor `22f620dc89b6999d88298ac70bde36591168c86e`; #875's stack merge invalidated transfer of that exact-head evidence. Acceptance requires the same evidence on one stable current exact head plus terminal gates and counted review. |
...
| [#867](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/867) | `3cd34813323d846573cf22480038cf545adde26d` / `fix/provider-backed-embedding-batch` | Normally merged into #857 as merge commit `b49d6ff1f439964671e979ae2202aa6bbedd572d`; protected delivery now follows #857's updated `main`-target exact head and its replacement checks, findings, and review. |
| [#868](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/868) | `5839c957193ec8ce21ec8a3d34a7663f698f84b4` / `main` | The moving head now includes the normally merged #870 conduct regression fix, #874 configured-gateway discovery stack, and follow-up boundary fixes. Runtime auto-discovery consumes injected pool configuration plus preseeded KV instead of environment secrets and isolates an invalid persisted gateway instead of aborting startup; configured-gateway catalog calls reuse the DNS-pinned, proxy/redirect-independent provider transport with a bounded body; and price, input, and output capability metadata require complete deployment consensus. A local exact-head focused run passed `59` tests and `git diff --check`; replacement hosted jobs and counted review remain pending, no review threads are unresolved, and GitHub reports `BLOCKED`. Auto-merge remains enabled only after that closure audit, but acceptance still requires terminal exact-head hosted gates, counted review, normal protected merge, and an authenticated runtime/default check. |
...
| [#874](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/874) | `12a2c55c227e41cc0f28dc235da65aef1351aee4` / `fix/gateway-default-chat-model` | Merged normally into stacked base #868 as `37dd6193e893fcf9e196afd6ac859b4e544e18af`, not into protected `main`. Its three remaining environment-secret, provider-egress, and heterogeneous-capability gaps were first fixed by predecessor #868 head `90cca8b531ed4e174fce13339e848466aced364a` and are retained by current #868 head `5839c957193ec8ce21ec8a3d34a7663f698f84b4`; child closure does not establish protected delivery. Acceptance follows #868's current exact-head gates and post-protected-merge authenticated catalog check. |
| [#875](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/875) | `33a93db391c7d744c80cf4055d6a20a10ba76d2b` / `fix/provider-backed-embedding-batch` | Merged normally into stacked base #857 as `9ecf696803fa474cb0c2ad66c2acd4f854b05c56`. Durable readiness evidence is therefore present on #857's new exact head, but not on protected `main`; acceptance follows #857's replacement checks and counted review. |
| [#876](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/876) | `6bae530bbe9072c350b2cf4bf6c4b2296944bcda` / `main` | Installs the pinned OpenCode runtime binary for the hourly loop contract. Fifteen hosted jobs are pending, no review threads are unresolved, review is required, and GitHub reports `BLOCKED`. Acceptance requires terminal exact-head workflow evidence, counted review, normal protected merge, and a subsequent scheduled-run canary on the protected commit. |
| [#877](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/877) | `a39958105427e91b680197b55318f86d51e9792f` / `main` | Closed without merge after exact trust-boundary audit. Although malformed evidence was made fail-closed, the public report still accepted an internally consistent caller-authored dictionary of arbitrary SHAs, check policy/results, reviewer claims, and finding counts; the HTTP path supplied no trusted artifact and always blocked. This cannot satisfy issue #103. Acceptance remains the recorded ecosystem boundary: `.github` produces an immutable complete exact-head artifact, contextual-orchestrator verifies non-forgeable provenance/digest/signature/repository/head/base ancestry/freshness/completeness, and GRC archives the verified artifact. |
...
has a current-exact recovery matrix: constant-time budget counters, telemetry
session correlation, and request-scoped sampling are semantic replacements on
protected `main`; passthrough failover, structured-provider orchestration,
generated-workflow budget checkpoints, local-chat default token caps, and
empty-trace measured usage are owned by #851 exact
`d49846d5c8deca55c60315a8fbb461ecdebf67eb` (shared historical commits may also
appear in other stacks, but #851 is the delivery authority). The implementation
gaps are closed on that PR head with focused and official-full evidence; this is
semantic/test evidence, not transferred ancestry, checks, review, or protected
delivery. Issue #846 remains open until #851 completes hosted exact-head gates,
independent review, and a normal protected-main merge.
