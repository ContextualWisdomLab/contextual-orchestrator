...
| --- | --- | --- |
| [#843](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/843) | head `9a084d03edfccd7c46d1ee6ab05062af525622c7`, base #834 head `3a7f4a6d71286d13f55b7a339e7b746198e238c3` | Open stacked PR. `orchestrator/auto` and `orchestrator/free` are durable virtual models across text, image, video, speech, transcription, embeddings, rerank, and audio; `free` admits only discovery-tagged `cost:free`, exact agent-id zero prices, or an unambiguous legacy model-id zero price. Free routing uses the same measured ordering for primary, failover, tools passthrough, judging, and capability requests, records ungrouped capability outcomes, and fails closed when no eligible model exists. `/v1/responses` streams fixed safe stage summaries with OpenAI `response.reasoning_summary_part.*` and `response.reasoning_summary_text.*` events; raw chain-of-thought and intermediate output are excluded, caller instructions reach every conducted stage, structured-output requests fail closed, post-header failures terminate with `response.failed`, and transport/application status are recorded separately. URL-encoded provider and virtual model identifiers are retrievable from `/v1/models/{id}`. The hourly OpenCode loop now uses `orchestrator/auto`. Exact-head evidence is `1678 passed in 706.95s`, focused routing/failover tests `121 passed`, Actionlint passed, and the cwd-independent locked image built as `sha256:19b7d1cbda5678804721daa6cf936a5ffbeea4b3005fbbbf9be2329b42b77c3b`. All known Devin threads are resolved; exact-head re-review remains pending. |
| [#834](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/834) | head `3a7f4a6d71286d13f55b7a339e7b746198e238c3`, base current `main` | Parent model-group stack remains the required predecessor of #843. No protected-main or release claim transfers from local or downstream evidence. |
