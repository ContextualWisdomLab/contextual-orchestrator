...
| [#790](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/790) | `bfdcdd006365f6af2930ca4798806273e980bef6` | ready, based on main | Latest exact head keeps the gateway auth token outside the provider-key bootstrap gate, covers model-discovery rejection paths, and rejects `gte-*` embedding families from chat-capability roles. Exact-current-head focused capability/model-discovery/review-gateway proof is `81 passed`; all three changed source modules reached 100% statement and branch coverage in the local run, with Ruff/compileall/diff-check clean. Hosted Checks and independent approval remain required; predecessor-head evidence does not transfer. |
