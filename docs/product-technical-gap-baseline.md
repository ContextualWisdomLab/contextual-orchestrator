...
**As of:** 2026-08-21 10:29, Asia/Seoul
...
| [#790](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/790) | `d3a27db0a69f09f245a19a189ec41d3aa2f6b2fc` | ready, based on main | Latest exact head keeps the gateway auth token outside the provider-key bootstrap gate, covers model-discovery rejection paths, rejects `gte-*` embedding families from chat-capability roles, and keeps foreign-currency prices out of direct ranking. Exact-current-head focused capability/model-discovery/review-gateway proof is `65 passed`; all three changed source modules reached 100% statement and branch coverage in the local run, with Ruff/compileall/diff-check clean. Hosted Checks and independent approval remain required; predecessor-head evidence does not transfer. |
...
| [#801](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/801) | `1ed5148f92c2acf8cea5818ca39da437237c9696` | ready, stacked on [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) at `a4e4f683e3a4f39fc740b9028158da7e7c2bc219` | Explicit `argv` injection lets LineageWeave invoke the CLI without mutating process arguments. Exact-head proof is `9 passed` in CLI auth, with Ruff/compileall/diff-check clean; hosted Checks and independent approval remain required. |
...
At the refreshed snapshot, central scheduler PR [#1178](https://github.com/ContextualWisdomLab/.github/pull/1178) is open at exact head `97b084ac28b5ccf6de7f68fd2e019d8da6f80143`. Its target caller runs at minute `17`, uses non-cancelling concurrency, scans at most 50 PRs, dispatches at most one repair, and forwards only `PR_REVIEW_MERGE_TOKEN`/`OPENCODE_APPROVE_TOKEN`; current hosted Checks are pending and the cancelled queue scan is not a terminal failure. Related central gateway PR #1170 is open at exact head `84c7448f80d2aa06d5ab1b583bf640265160f565` and records target #790 at current exact head `d3a27db0a69f09f245a19a189ec41d3aa2f6b2fc`; its own protected Checks and independent approval remain pending. Superseded #1183 is closed without merge, so its head is not integration evidence.
