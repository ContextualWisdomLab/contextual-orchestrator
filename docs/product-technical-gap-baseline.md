...
| P1 | Psychometric routing lacks a held-out accuracy-time acceptance gate. | Local performance commit `0561c9b81f68bd9be9bd413f4a23892717e54701`, carried by [#1058](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/1058), reduces fixed-fit gateway preparation median from 2.448167 ms to 1.100583 ms while 30 focused tests pass. This is candidate latency evidence only; it does not measure answer accuracy and is not protected-main evidence. | On a preregistered held-out model-query matrix, report log loss or Brier score, top-choice regret, true-parameter RMSE where available, route-decision p50/p95, and end-to-end p95. Admit a policy change only when accuracy is non-inferior or better, latency improves, and protected exact-head checks pass. |
