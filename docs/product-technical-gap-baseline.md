...
| P1 | Psychometric routing lacks a buyer-held-out accuracy-time acceptance gate. | Local performance commit `0561c9b8`, carried by [#1058](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/1058), reduces fixed-fit preparation median from 2.448167 ms to 1.100583 ms. Stacked [#1061](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/1061) adds a seeded 24-training/24-held-out semantic warm-start experiment: `50d91c9e` reduces expected Brier 0.1438369123 to 0.1418346845, log loss 0.4525311878 to 0.4475784303, and mean top-choice regret 0.0024259478 to zero while decision p50 stays near 0.02 ms. These are synthetic candidate results, not protected-main or buyer-prompt evidence. | Run the same versioned metric contract on a preregistered buyer model-query matrix and add end-to-end p95 plus true-parameter RMSE where available. Admit a policy change only when accuracy is non-inferior or better, latency improves, and protected exact-head checks pass. |
