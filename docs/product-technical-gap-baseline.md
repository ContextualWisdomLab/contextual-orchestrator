...
| [#789](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/789) | `7ae2644c337b66595ab304fbe35c3742a492272a` | open, based on main; the replacement head has restarted hosted workflows and remains `REVIEW_REQUIRED` | Embedding requests may omit `model`; the existing ranking policy resolves an enabled embedding agent. Runtime auto-discovery now activates only models with source-declared `chat` capability, so an embedding-only or unclassified deployment cannot become a chat worker by model-name inference. Exact-head local evidence is `1446 passed`; hosted checks and an independent approval remain required. |
...
| P2 | LineageWeave has no protected-main consumer acceptance gate. | [#801](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/801) added explicit CLI `argv` only to a non-main stack; documented main still exposes `contextual_orchestrator.__main__.main()` without an `argv` argument. The generic consumer row below cannot prove LineageWeave no longer monkey-patches process arguments. | Promote an explicit CLI invocation contract to protected main, then run a LineageWeave-owned authenticated end-to-end test through the OpenAI-compatible endpoint without mutating process arguments; retain the authorization and chat-completion evidence against that exact main SHA. |
...
- Storybook/`ui-ux-pro-max`/`Anti-Slop-UI` are deliberately deferred, not
  skipped: [ADR 0011](planning/adrs/0011-admin-console-ui-tooling-boundary.md)
