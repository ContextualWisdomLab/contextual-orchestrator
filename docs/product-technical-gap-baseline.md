...
**As of:** 2026-08-21 09:23, Asia/Seoul
...
| [#796](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/796) | `7af40759ca7fb941834af4eeca8197bd2bfe3dba` | ready, based on [#795](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/795) | Cost-ledger normalization separates execution facts from attribution dimensions, keeps migration transactional, enables SQLite foreign keys before schema work, maps nullable legacy attribution to `unattributed`, and rolls back failed append writes. Exact-head proof is focused `36 passed`, full `1453 passed in 524.20s`, compileall, and diff-check clean; it includes static migration SQL, seeded-catalog rollback, FK enforcement/cascade, PostgreSQL metadata selection, qualified SQL naming, and failed-append rollback. Hosted Checks and independent approval remain required. |
| [#794](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/794) | `d5f69a71281394fe5af74a9e510d31358df1cfd4` | ready, based on main | Database naming repair renames the single-word state table to `orchestration_records`, preserves legacy rows through an atomic fail-closed migration, uses static migration DDL to satisfy SQL-safety scanning, closes the connection on schema failure, and covers table/index/view/sequence/constraint forms including quoted and schema-qualified identifiers. Exact local proof is focused `12 passed`; the previous full-suite evidence belongs to the superseded parent head. Hosted Checks and independent approval remain required. |
| [#795](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/795) | `cb0d9a0592c66ba7895c3ebca302238c82e75e6a` | ready, based on [#794](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/795) | Durable agent-pool storage is normalized into scalar, ordered-tag, and provider-exclusion tables; legacy JSON migration remains transactional, every SQLite connection enables foreign-key enforcement before work begins, and the current #794 qualified/quoted/inline-constraint naming-gate repair is included in the head tree. The current head is included in #796's exact `1453 passed` stack proof; hosted Checks and independent approval remain required. |
...
| [#784](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/784) | `d246e675156a22639dec4218e1bb226a3ea5c282` | ready, stacked on [#782](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/782) | Current head preserves product release-artifact blockers without double-counting concrete blockers already included in the product count, rejects non-built-in string subclasses that can bypass author exclusion, and includes #782's owner-bound workflow/evaluation evidence fix to satisfy the Strix IDOR gate. Exact proof is focused `106 passed`, compileall and diff-check clean; hosted Checks are queued on this exact SHA and protected approval remains required. |
...
| [#770](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/770) | `6b603efeb9728d7c142f090153925948c0f1248f` | ready, based on [#768](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/768) | Current stack consumes the shared ordinary-chat classifier and price-honest provider-diverse selection. Hosted Checks are pending on this exact head; obtain an independent current-head approval before protected merge. |
...
| [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | `ba8b4b8ab181bcd2c64b871d0984006452e4c753` | ready, based on main | Latest head removes the runtime sampling monkey patch on top of the virtual-model embedding-capability gate. The earlier `1510 passed` proof belonged to predecessor `2ab9b9f011403bcea7ee812c5b5b389ddebe1181` and does not transfer; hosted Checks are queued on this exact head and independent approval remains required. |
...
observed at 2026-08-21 09:23 Asia/Seoul; they are evidence pointers, not
standing approval.

### External central workflow prerequisite

At the refreshed snapshot, ContextualWisdomLab/.github PR [#1170](https://github.com/ContextualWisdomLab/.github/pull/1170) is open at exact head `4684f6e212ba40d12e5217f0f52ee1e90c796ed8`, while central main remains `55a8b576725451dfe0a21a57d36a2f1a41619b24`. It pins contextual-orchestrator target #790 at `0071751782ae535721e71785c3037989d2d27b77`, keeps the `PR_REVIEW_MERGE_TOKEN`/`OPENCODE_APPROVE_TOKEN` publication boundary, preserves provider fallbacks, and explicitly omits `COPILOT_GITHUB_TOKEN`; current hosted Checks are pending and independent approval is absent. Superseded #1183 is closed without merge, so its head is not integration evidence.
