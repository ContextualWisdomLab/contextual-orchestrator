...
**As of:** 2026-08-21 02:33, Asia/Seoul
...
| [#788](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/788) | `d670601e61ca181a7b7134c7d0219f310334ff05` | ready, based on main | Review opaque admin-session TTL/revocation, same-origin cookie state changes, and Secure-by-default deployment. Historical Strix run `32376890077` failed closed because NVIDIA NIM rate-limits prevented a structured report; the deleted workflow cannot be rerun directly, so fresh central exact-head Checks remain required. |
| [#789](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/789) | `02e13a84bf4149e56d3bc6ee5cef41d8b13b2d4f` | ready, based on main | Review optional embedding model selection, capability-constrained pool validation, OpenAPI parity, [ADR 0024](https://github.com/ContextualWisdomLab/contextual-orchestrator/blob/02e13a84bf4149e56d3bc6ee5cef41d8b13b2d4f/docs/planning/adrs/0024-auto-embedding-model-selection.md), and fresh exact-head protected Checks before normal merge. |
...
| [#780](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/780) | `65c29809d4a19163bf083a4f1a9204f70183bc74` | ready, based on main | Parent-integrated head includes #781 trace-purpose authorization. Review the minimal `/healthz` contract, authenticated `/readyz`, optional-dependency degradation, backend-identifier non-disclosure, ADRs 0025/0026, and fresh exact-head hosted Checks before normal merge. |
| [#778](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/778) | `f696daa534a0fcda12d2af8bc3855058a89d6cd9` | ready, stacked on [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | Current-parent head preserves source images, explicit `vision` eligibility/failover, Responses normalization, the private LineageWeave OCR recovery boundary, and deadline-boundary fixes; integrated focused suite is `61 passed`. |
...
| [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | `61337d9007bb66a77381dc8d8429ad78d968c301` | draft, based on main | Current parent includes the merged #779 temperature-negotiation stack at merge commit `a4e9a780a56c08e131367f3b904f5c855b3a1b33` plus the complete-body-at-deadline fix; exact framing/deadline proof is `20 passed`. Reconcile after [#768](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/768)/[#769](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/769). |
...
observed at 2026-08-21 02:33 Asia/Seoul; they are evidence pointers, not
standing approval.
...
The scheduler's earlier dry-run decision for PR #787 was `review_dispatch` for its non-main base; #787 and #779 are now merged into the #765 parent stack. The target repository currently exposes no `opencode-review.yml` workflow of its own and the central repository-dispatch queue remains asynchronous. Until #1170 reaches protected main, remaining stacked PR review may remain absent or use the prior central model path; this is an external integration dependency, not evidence that the stacked code is merge-ready.
