...
| [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | `feed7184872fb5eedb48e277ccf9ed09c46143f3` | ready, based on main | Exact head rejects empty provider URL userinfo and literal empty fragments in both discovery and shared provider JSON transport, with an explicit split-token admin GET regression. Local exact-head proof is focused `41 passed`, full `1515 passed in 563.50s`, compileall, and diff-check clean. Hosted Checks are queued on this exact head and independent approval remains required; predecessor Strix results do not transfer. |
