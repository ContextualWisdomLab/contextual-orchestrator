...
| [#851](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/851) | `218b8d9d26476786b15f1d455739876064b4d2ec` / `main` | The moving head includes #871's measured equivalent-endpoint race and consistent provider attribution for completed race-loser spend. Sixteen replacement hosted jobs are pending, no review threads are unresolved, review is required, and GitHub reports `BLOCKED`; earlier full-suite evidence does not transfer. Provider failover and recovered #846 work are not protected delivery until every required job is terminal-successful and review policy is satisfied. The #846 generated-workflow budget-checkpoint and local-chat default-token gaps remain open because this head movement changes only race accounting. |
...
| [#877](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/877) | `bac872fd98ca4946000789087906951c7462d9d7` / `main` | Adds fail-closed exact-head commercial release authorization and its ADR/tests. Fifteen hosted jobs are pending and four review threads remain unresolved, including one reported malformed-check-name crash and three intent confirmations; GitHub reports `BLOCKED`. Acceptance requires resolving the genuine malformed-evidence path without weakening fail-closed authorization, closing the informational threads with ADR evidence, then obtaining terminal exact-head gates and counted review. |
...
protected `main`; passthrough failover and structured-provider orchestration are
owned by #851 exact `218b8d9d26476786b15f1d455739876064b4d2ec`
(their shared historical commits also appear in #857, but #851 is the delivery
authority). Two minimal gaps remain: generated workflows do not yet checkpoint
and stop between provider calls when an in-flight budget is reached, and local
chat passthrough does not restore the configured default `max_tokens` when the
caller omits it. The orphan commit IDs are ancestors of none of current
protected `main`, #851, or #857, so this mapping is semantic/test evidence—not
transferred ancestry, checks, or review. Each remaining gap closes only with
