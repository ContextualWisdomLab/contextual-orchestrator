...
**As of:** 2026-08-21 08:39, Asia/Seoul
...
| [#796](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/796) | `9f80c03d0ec24b58210a3c0024d03771d4e4c30e` | ready, based on [#795](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/795) | Cost-ledger normalization separates execution facts from attribution dimensions, keeps migration transactional, enables SQLite foreign keys before schema work, selects PostgreSQL metadata through the driver-native path, maps nullable legacy attribution to `unattributed` before `NOT NULL` insertion, and includes the latest #795 static-DDL base in the head tree. Legacy migration now starts an explicit transaction before DDL and has a seeded-catalog mid-copy rollback regression. Exact-head proof is focused `44 passed`; full-suite evidence from the superseded tree must be revalidated on this SHA. Hosted Checks and independent approval remain required. |
...
| [#795](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/795) | `dbe03da58ca45f0747d8b27a7e9079049ceb9011` | ready, based on [#794](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/794) | Durable agent-pool storage is normalized into scalar, ordered-tag, and provider-exclusion tables; legacy JSON migration remains transactional, every SQLite connection enables foreign-key enforcement before work begins, and the current #794 qualified/quoted/inline-constraint naming-gate repair is included in the head tree. Exact local proof is focused `22 passed`; full-suite evidence from the superseded parent tree must be revalidated on this SHA. Hosted Checks and independent approval remain required. |
...
observed at 2026-08-21 08:39 Asia/Seoul; they are evidence pointers, not
standing approval.
