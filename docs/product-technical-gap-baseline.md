...
| [#868](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/868) | `90cca8b531ed4e174fce13339e848466aced364a` / `main` | The moving head now includes the normally merged #870 conduct regression fix, #874 configured-gateway discovery stack, and three follow-up boundary fixes. Runtime auto-discovery consumes injected pool configuration plus preseeded KV instead of environment secrets; configured-gateway catalog calls reuse the DNS-pinned, proxy/redirect-independent provider transport with a bounded body; and price, input, and output capability metadata require complete deployment consensus. A local exact-head focused run passed `58` tests and `git diff --check`; sixteen hosted jobs remain pending, no review threads are unresolved, review is required, and GitHub reports `BLOCKED`. Auto-merge was re-enabled only after that closure audit, but acceptance still requires terminal exact-head hosted gates, counted review, normal protected merge, and an authenticated runtime/default check. |
