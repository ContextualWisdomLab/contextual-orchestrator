...
| --- | --- | --- |
| [#851](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/851) | `d71bafc4c6ad2152a065161dc7e3c9bd464d5fab` | Recovers only the current-main passthrough-failover slice from #846. Virtual-model tools, structured-output, and Responses payloads advance once per distinct provider after stale/transient failures; concrete models remain sticky, permanent DNS/caller errors fail closed, and only `EAI_AGAIN` DNS failures advance. The predecessor exact tree passed all `1886` tests; the review-repaired head passed `63` focused tests, so replacement hosted full-suite evidence is required. Independent review and protected checks remain pending with auto-merge enabled. |
