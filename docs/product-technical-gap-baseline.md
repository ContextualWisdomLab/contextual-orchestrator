...
| [#790](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/790) | `172a8901f6fa6cad500917095e92878ed9ffcf04` | ready, based on main | Latest exact head keeps the gateway auth token outside the provider-key bootstrap gate and adds model-discovery rejection-path coverage. Exact-current-head focused model-discovery/review-gateway proof is `50 passed`; `model_discovery.py` and `review_gateway.py` each reached 100% statement coverage in the local run, with Ruff/compileall/diff-check clean. Hosted Checks and independent approval remain required; predecessor-head evidence does not transfer. |
