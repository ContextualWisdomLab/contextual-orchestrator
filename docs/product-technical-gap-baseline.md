...
| --- | --- | --- |
| [#845](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/845) | head `19a29246683fbb47e37d916910953b3e2342af36`, base `main` `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | Repairs hourly provider-catalog runs `32834283714` and `32829694684`, which failed before discovery because both durable KV secrets were absent. The workflow now uses durable PostgreSQL only when both secrets exist, uses an explicitly run-scoped PostgreSQL service when neither exists, fails closed on partial configuration, pins its container digest, and does not copy durable secrets through `GITHUB_ENV`. `actionlint` and the focused `4 passed` contract are clean; hosted exact-head Checks and independent approval remain authoritative. Decision: `WAIT_AND_REMEDIATE`. |
| [#834](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/834) | head `a11c541a991c47ae608ba44cdd0292537abec492`, base `main` `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | Open with normal auto-merge enabled. Zen availability is joined to Models.dev structured cost/modality evidence without name inference. Normalized catalog reload distinguishes provider-declared `capability:*` evidence from generic serving-role tags and preserves cost/modality evidence. Explicitly excluded embedding deployments fail closed; operator-declared Bytez endpoint capabilities survive discovery without model-name inference. PATCH/DELETE now share the GET `model_group_not_found` contract. Focused evidence includes `92 passed`, `59 passed`, `48 passed`, and the subsequent HTTP regression; hosted Checks and independent exact-head approval remain authoritative. Decision: `WAIT_AND_REMEDIATE`. |
| [#818](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/818) | head `860a4ff75183d4d2f5c0b4b80d86db23526abb96`, base `main` `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | The OpenTelemetry slice is reconciled with current-main request, streaming, batch, dependency-lock, and session-security paths. Reauthorization replaces inbound trace context instead of stacking it. The first full run found one CI-contract failure after `1846` passes; the exact-head repair installs both runtime and property locks, removes the superseded attribute denylist, and its focused `20 passed` regression is clean. A fresh full hosted suite and independent review are required. Decision: `WAIT_AND_REMEDIATE`. |
...
| #818 | `860a4ff75183d4d2f5c0b4b80d86db23526abb96` | `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | `BLOCKED`, review required, auto-merge enabled; full-suite CI dependency contract repaired after one exact-run failure |
| #834 | `a11c541a991c47ae608ba44cdd0292537abec492` | `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | `BLOCKED`, auto-merge enabled; missing-group REST error contract normalized |
| #845 | `19a29246683fbb47e37d916910953b3e2342af36` | `6970dbb9e63b9bd1ec602bb8c3c85e3a05480424` | `BLOCKED`, review required, auto-merge enabled; scheduled provider sync root cause repaired |
