...
**As of:** 2026-08-21 02:19, Asia/Seoul
...
| [#778](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/778) | `915f780aec8d7d0ae6a35141a221aec9b9cbc40c` | ready, stacked on [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | Current-parent head preserves source images, explicit `vision` eligibility/failover, Responses normalization, the private LineageWeave OCR recovery boundary, and inbound deadline fixes; integrated focused suite is `58 passed`. |
| [#779](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/779) | `ef7b1fb777b056d8180b3dffac2f2abbb8cdb666` | ready, stacked on [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) | Parent-integrated head includes #787, the Responses tool-loop stream/input fix, and total inbound-body deadline enforcement. ADR 0016 is unique; the focused integration suite is `38 passed`. Revalidate this exact head and integrate only after [#765](https://github.com/ContextualWisdomLab/contextual-orchestrator/pull/765) reaches protected main. |
...
The scheduler's earlier dry-run decision for PR #787 was `review_dispatch` for its non-main base; #787 is now merged into the #779 parent stack. The target repository currently exposes no `opencode-review.yml` workflow of its own and the central repository-dispatch queue remains asynchronous. Until #1170 reaches protected main, remaining stacked PR review may remain absent or use the prior central model path; this is an external integration dependency, not evidence that the stacked code is merge-ready.
