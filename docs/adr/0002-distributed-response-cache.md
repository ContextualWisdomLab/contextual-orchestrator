...
mode, and ordered message envelope. Prompts and responses do not appear in the
remote key. Cache reads and writes fail open because the cache is an optimization,
not the source of truth. `X-Cache-Bypass: true` (or `1`) bypasses the cache for a
request; ambiguous header values are rejected.
