...
- `composed_catalog.compose_default_catalog`: when a KV credential is present, discover chat models from the provider list API using the same public-HTTPS / no-redirect egress policy as chat; use a static fallback only if that GET fails.
