...
- `contextual_orchestrator.server`: small `/v1/chat/completions` HTTP server.
  ...
  participant `name`, a non-empty `messages` array, assistant `tool_calls`
  entry/function keys (`id` / `type` / `function` / optional `index` only),
  and `max_tool_calls` are validated *before* the tools/response_format
  passthrough early-return so SDK tool-calling bodies cannot smuggle
  unsupported values upstream. Omit-equivalent `max_tool_calls` (JSON null /
  empty string) is stripped, not forwarded. The same early-return fail-closes
  `stream=true`, requires a pool `model`, range-checks sampling knobs, and
  rejects unknown spend keys and batch routing hints. Request sampling knobs
  (`temperature`, `top_p`, penalties, `max_tokens`) are applied via
  `ModelClient.request_sampling` on the calling thread only so concurrent
  Completions/chat requests cannot observe each other's knobs.
