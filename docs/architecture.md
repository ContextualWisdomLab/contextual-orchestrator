...
- `Orchestrator.conduct`: the workflow path with planner, worker, verifier, and synthesizer steps. A rejected conduct run serves the same public rejection envelope as verify — not the worker or synthesizer text.
