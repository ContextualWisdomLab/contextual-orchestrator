...
- `contextual_orchestrator.server`: small `/v1/chat/completions` HTTP server. Message `name` is omit-validated before tools passthrough so blank SDK defaults never smuggle to a provider.
