...
- `contextual_orchestrator.server`: small `/v1/chat/completions` HTTP server. Tools and `response_format` take the single-agent passthrough path; `stream=true` on that path is an SSE proxy, not a `400`. Omit-equivalent `max_completion_tokens` still validates sibling `max_tokens`, so `max_tokens=0` cannot bill. `top_logprobs` must be an integer; JSON `false` is `invalid_top_logprobs`, not omit-as-zero. `exercise_output_token_budget` in `fuzz/targets.py` drives those two helpers on arbitrary decoded bodies.
