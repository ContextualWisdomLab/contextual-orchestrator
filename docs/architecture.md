...
- `GET /v1/models` (and `/v1/models/{id}`): OpenAI-compatible model discovery from the agent pool (inference scope) so SDKs and consumers can list selectable model ids without admin access.
