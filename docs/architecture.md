...
- `contextual_orchestrator.semantic_chunking`: meaning-unit cuts for `/v1/batch/embeddings` when `chunking_strategy=meaning_units` (email parties, innermost HTML leaves, RFC 2397 / RFC 2045 embedded-image offsets including 76-column folds, paragraphs). Token-budget map/reduce stays the provider-safety path and still averages parts of *one* unit.
