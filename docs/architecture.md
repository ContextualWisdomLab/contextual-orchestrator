...
- `Orchestrator.route_and_verify` (`mode="verify"`): one worker call plus one checked verifier judgment — for adjudication-shaped requests that need a verified verdict without the full four-step workflow's cost. Term matching is word-bounded and fail-closed: `password`, `looks good`, and `I have not accepted this` are not accept verdicts. Auto-mode only bills verify for explicit adjudication tokens (`verify`, `adjudicate`, `검증`, `심사`), not everyday `validate` / `judge` / `확인` / `평가`. A rejected verify or conduct result returns a rejection envelope; worker text stays on the trusted trace. Stream chunks redact verification the same way as the sync completion.
