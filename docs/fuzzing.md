...
1. **HTTP request body** — `server._coerce_json` / `_reject_unknown_keys` /
   `_validate_mode` / `_validate_messages` / `_parse_content_length`. Arbitrary
   bytes must normalise to a validated structure or raise `RequestError` / a
   JSON decode error — never an unhandled crash. Arbitrary `Content-Length` /
   `Transfer-Encoding` values must return a non-negative size within
   `max_body_bytes` or raise `RequestError` — never `read(-1)`.
