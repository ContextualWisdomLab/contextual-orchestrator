...
(`NotConfigured`) and does **not** fall back to GitHub Models. The sidecar
workflow fails if `/healthz` never succeeds; the no-secrets smoke skip happens
only after a healthy serve.
