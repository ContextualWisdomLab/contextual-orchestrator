...
The implementation asks only agents explicitly marked
`stream_usage_supported=true` for the Chat Completions terminal usage frame,
keeps provider usage on each completed workflow trace step, records it through
the existing cost ledger, and returns standard Responses
