...
| Sequential drift delay | Chen, Lee, and Li (2022) frame item drift monitoring as sequential change detection with compound false- and missed-detection risk. | Source `fafa7bee` runs a narrower seeded one-stream Bernoulli CUSUM screen. A probability shift from `0.8` to `0.3` after 100 observations triggers at observation 107 with no earlier alarm. Buyer time-series observations, risk weights, and preregistered false-alarm and delay targets remain required. |
