...
| Sequential drift delay | Chen, Lee, and Li (2022) frame item drift monitoring as sequential change detection with compound false- and missed-detection risk. | Source `7c3e6e98` runs a narrower one-stream Bernoulli CUSUM screen over 500 repetitions. Raising its threshold from `log(100)` to `7.0` reduces false alarms from `21.2%` to `1.4%`, while delay p50 rises from 6 to 10 observations and p95 from 17 to 22. Buyer time-series observations, risk weights, and preregistered targets remain required. |
