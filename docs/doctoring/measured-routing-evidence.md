...
| Classification-oriented stopping | Luo, Kim, and Dickison (2018) describe CI stopping around an explicit decision cut and its near-cut inefficiency. | Source `f4cceb59` stops when a 95% normal interval excludes cut zero or after 12 queries. Across 400 known synthetic candidates it averages 9.875 queries, stops early for 41%, and exactly matches the fixed-length decisions and 0.9125 accuracy. Paired intervals are `[-2.425, -1.835]` queries and `[0, 0]` accuracy. Source `298e1fc8` exposes the heterogeneity: within 0.5 of the cut, early stopping is 3%, mean length 11.86, and accuracy 0.70; at least 1.0 away, they are 68%, 8.305, and 1.0. Source `e2cb547f` keeps unresolved cases explicit: 42.5% are confidence-resolved and all resolved synthetic decisions are correct, while only 3% of near-cut cases resolve. This is selective synthetic accuracy, not permission to force or deploy the remaining decisions. Normal-interval calibration, buyer costs, subgroup guarantees, and live latency remain unvalidated. |
