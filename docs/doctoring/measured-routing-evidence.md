...
| Predictive-fit axes | Stenhaug and Domingue (2022) distinguish missing-response prediction for observed persons from complete-response prediction for new persons. | Source `68831dff` labels the existing synthetic held-out-query Brier and log-loss evidence as the known-candidate task and reports unseen-candidate prediction separately as `not_executed`. Versioned buyer outcomes for both axes remain required. |
