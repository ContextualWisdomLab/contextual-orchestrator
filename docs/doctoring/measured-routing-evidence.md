...
| Classification-oriented stopping and rejection | Luo, Kim, and Dickison (2018) describe CI stopping around an explicit decision cut and its near-cut inefficiency; Chow (1970) and El-Yaniv and Wiener (2010) frame abstention as an error–coverage tradeoff. | Source `e1ff2e61` chooses `z=1.645` on a development seed under a 2.5% Wilson error-upper-bound rule. On one paired holdout it raises coverage from 44.25% to 56% and lowers all-candidate query burden from 9.88 to 8.395. Source `609faff8` prevents that one run from authorizing the candidate: across ten independent seeds, coverage gain remains 9.75–15 points and query reduction 1.27–1.485, but selective risk reaches 1.802% and its Wilson upper bound reaches 4.540%; only 20% of runs satisfy the declared 2.5% ceiling. The candidate is rejected as replication-unstable. Buyer fallback policy, calibrated intervals, and live latency remain unexecuted. |
