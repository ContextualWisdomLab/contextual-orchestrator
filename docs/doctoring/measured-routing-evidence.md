...
| Classification-oriented stopping | Luo, Kim, and Dickison (2018) describe CI stopping around an explicit decision cut and its near-cut inefficiency. | Source `f4cceb59` stops when a 95% normal interval excludes cut zero or after 12 queries. Across 400 known synthetic candidates it averages 9.875 queries, stops early for 41%, and exactly matches the fixed-length decisions and 0.9125 accuracy. Paired intervals are `[-2.425, -1.835]` queries and `[0, 0]` accuracy. Normal-interval calibration, near-cut risk, buyer costs, and live latency remain unvalidated. |
