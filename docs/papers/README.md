...
`docs/doctoring/cost_performance_routing.md`) for how live-discovered tagged
workers are chosen (quality per unit cost) and when a Conductor workflow is
required. The static seed is fallback only.
