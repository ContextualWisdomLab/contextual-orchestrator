...
`logprobs=true`, boolean or nonzero `top_logprobs` / `logit_bias`, or
out-of-range penalties / `max_tokens`. A null `max_completion_tokens`
does not hide `max_tokens=0`. `stream=true` with tools is `invalid_stream`
