...
| `POST` | `/v1/batch/embeddings` | Submit a bulk, latency-tolerant embeddings batch (routed via pg-llm-batch, cost-recorded) |
| `GET` | `/v1/batch/embeddings/{batch_id}` | Poll an embeddings batch; returns vectors + recorded cost once completed |
