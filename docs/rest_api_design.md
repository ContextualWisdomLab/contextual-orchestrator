...
| `POST` | `/v1/responses` | OpenAI Responses passthrough. Send a non-empty `input`. Leave `instructions` out, or send JSON `null` / empty / whitespace — the gateway removes the key before upstream so SDK optional defaults do not become a blank system prompt. Non-string or >32000-character `instructions` return `invalid_instructions`. Official `text.format` (`type` = `text` / `json_object` / flat `json_schema`) is accepted and forwarded. Do not send `text.verbosity` or both `text` and `response_format` — those return `invalid_text`. |
...
Official Responses clients send `text.format` as `{format: {type: "text"}}` and may send `json_object` or flat `json_schema` (`type` / `name` / `schema` / optional `description` / `strict`) on the same plane (OpenAI, 2024). The gateway forwards those objects. JSON-null or blank `description` and JSON-null `strict` are popped before proxy. If you see `invalid_text`, you sent `text.verbosity`, an unknown key, or both `text` and `response_format` — send one official `text.format` object and retry.
...
Wright, A., Andrews, H., Hutton, B., & Dennis, G. (2022). *JSON Schema: A media type for describing JSON documents* (Internet-Draft draft-bhutton-json-schema-01). Internet Engineering Task Force. https://datatracker.ietf.org/doc/html/draft-bhutton-json-schema-01
