...
whitespace-only values on any of those keys, mixed `orchestration=route`
plus spaces, `mode=conduct`, non-boolean
...
matching the orchestration `or` chain. Each alias key is checked on its
own so a leftover `or` chain cannot hide conduct or spaces.
