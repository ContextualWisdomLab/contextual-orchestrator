...
| Option | Decision | Why |
|---|---|---|
| Official OpenAI Chat Completions + Structured Outputs docs | Cite (APA 7th) in `docs/papers/README.md`; do not vendor PDFs | Authoritative field list: `name`, `description`, `schema`, `strict`; `name` charset `[a-zA-Z0-9_-]{1,64}` (ASCII; `str.isalnum()` is not the regex) |
