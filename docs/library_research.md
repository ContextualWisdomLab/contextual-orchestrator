...
| Area | Library | Decision | Evidence |
|---|---|---|---|
| Upstream model list | OpenAI-compatible `GET /v1/models` via `ModelClient.fetch_provider_json` after `provider_base_url_rejection` | Do not add LiteLLM, instructor, `requests`, or a second urllib client. Discover when a KV credential is present; static fallback only on failure. | Chat already pins public HTTPS before Bearer; discovery is the second caller (Ponytail extract) and reuses `_open_provider`. A flag-gated catalog seed is a different slice (PR #642). |
