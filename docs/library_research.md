...
| Structured-output validation | [jsonschema](https://python-jsonschema.readthedocs.io/) | Use the maintained Draft 2020-12 validator for orchestrated `json_schema` responses. | Python's standard library parses JSON but does not implement JSON Schema vocabularies, references, or validation semantics. Reusing `jsonschema` avoids a security-sensitive partial validator and preserves provider-independent validation. |
...
- Do not implement a partial JSON Schema validator; reuse `jsonschema` for the
  Draft 2020-12 contract already exposed by the OpenAI-compatible boundary.
