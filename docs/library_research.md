...
| Structured-output validation | [jsonschema](https://python-jsonschema.readthedocs.io/) | Use the caller-declared validator dialect and fail closed after one traced repair. | Python's standard library parses JSON but does not implement JSON Schema dialect selection or validation; `jsonschema` provides the maintained validator registry required by ADR 0035. |
