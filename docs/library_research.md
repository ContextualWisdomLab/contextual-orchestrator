...
| Structured-output validation | [jsonschema](https://python-jsonschema.readthedocs.io/) | Use the maintained Draft 2020-12 validator for provider-returned JSON against caller-supplied `json_schema` contracts. | Reusing `validator_for`, schema checks, and bounded validation avoids a security-sensitive partial validator. Provider output and schemas remain untrusted and fail closed. |
