...
Selected: stdlib regular expressions plus one forward HTML stack walk,
ASCII-only tag case folding, tag-only, and RFC 2045 image-fold walks
(innermost-leaf emission, a linear tag-only scan, then a payload walker
that stops at padding / space / quote / ``>``) and paragraph/email
detectors. Custom code that was deliberately skipped: LLM-as-chunker,
perplexity chunking, a backtracking ``.*?`` HTML matcher, a repeating
tag-only ``(?:\\s*</?[A-Za-z][^>]*>\\s*)+`` matcher, ``str.lower()`` for
tag offsets, a ``{16,}`` MIME continuation floor, and any new pip
dependency.
