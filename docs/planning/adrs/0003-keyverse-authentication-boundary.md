...
| Container startup passed a bearer secret as argv/env material, bypassing the KV boundary. | Pass only the split credential names `CONTEXTUAL_ORCHESTRATOR_ADMIN_TOKEN` and `CONTEXTUAL_ORCHESTRATOR_INFERENCE_TOKEN`; let the Keyverse/KV deployment adapter resolve their values at runtime. | Implemented |
...
| A specific non-loopback bind could bypass the wildcard-only public-bind check. | Require `--allow-public-bind` for every non-loopback address; keep loopback and `localhost` available for local development. | Implemented on current branch; protected-main integration required |
| A public/production CLI deployment could still select the legacy single bearer and thereby share answer and trace authority. | Keep single-token mode for explicit local development, but make `--production` and `--allow-public-bind` require split admin/inference credentials and reject insecure admin-session cookies; canonical Compose uses the split KV names. | Implemented on current branch; protected-main integration required |
