...
| A public/production CLI deployment could still select the legacy single bearer and thereby share answer and trace authority. | Keep single-token mode for explicit local development, but make `--production` require split admin/inference credentials and reject insecure admin-session cookies; canonical Compose uses the split KV names. | Implemented on current branch; protected-main integration required |
