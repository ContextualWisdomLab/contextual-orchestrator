...
| GitHub Dependabot still reports five open central `.github` alerts for `cryptography`/`aiohttp`, although both lock files pin the published patched versions (`cryptography==50.0.0`, `aiohttp==3.14.3`). | Do not dismiss security alerts merely because the lock appears fixed. Central commit `ed666c7c` adds a regression assertion covering both lock files and the patched floors; keep the alert-state discrepancy visible and close alerts only after GitHub refreshes manifest evidence. | CI floor assertion implemented; Dependabot refresh still required |
