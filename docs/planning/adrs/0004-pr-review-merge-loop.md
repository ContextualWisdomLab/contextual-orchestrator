...
| The fast-mlsirm full suite exposed an avoidable `RuntimeWarning: overflow encountered in exp` in the NumPy marginal reference `_log_sigmoid`: `np.where` evaluated both mathematically stable branches. | Use the same branch-stable formulation as the Rust core (`-np.logaddexp(0, -x)`), add an extreme-value regression test with overflow promoted to an error, and rerun the complete fast suite before merge. | Implemented on fast working branch `codex/fast-numerical-stability`; requires independent review, exact-head CI, and final re-fetch |
