...
| The provider host allowlist is still read from `CONTEXTUAL_ORCHESTRATOR_ALLOWED_PROVIDER_HOSTS` at request time, which is runtime configuration outside the KV boundary. | Move non-secret provider policy into an explicit KV/config record or constructor-injected immutable configuration; retain environment variables only for bootstrap transport and add a regression proving request-time env changes do not alter policy. | Goal expanded 2026-08-13; required follow-up |
