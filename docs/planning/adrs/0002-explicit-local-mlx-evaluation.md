...
| The HTTP gateway admitted only eight simultaneous orchestration runs even when a measured local MLX batch used `local_concurrency=16`, so the optimized batch setting could be hidden behind the server semaphore. | Preserve the secure default of eight, expose an explicit bounded `--max-concurrent-runs` CLI setting, and require operators to tune it separately with the local batch setting; never raise the default automatically or treat throughput as quality. | Implemented locally; CLI regression and exact-head review/check follow-up required |
