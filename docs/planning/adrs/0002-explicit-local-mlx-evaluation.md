...
| A conducted request expanded a large discovered catalog and attempted unprobed providers while trying to rank workflow roles, exhausting the caller deadline before a viable structured member was established. | Admission and discovery are separate: a conducted request may rank and call only members with current structured-readiness evidence (deterministic mock transports are the test-only exception). An empty admitted set returns typed `no_viable_agent` without a provider completion. Broad readiness refresh belongs to an explicit, bounded, asynchronous administrative job; it never runs inside discovery or a buyer request. The job requires an explicit agent access list and capability, uses the existing durable registry and provider-owned concurrency, is single-flight by canonical scope, supports deadline/cancellation/progress, and restores completed structured evidence after restart only while its persisted completion instant remains inside the readiness window. | Request-path admission and durable administrative refresh implemented with 455-unprobed, two-ready failure-bound, freshness-preserving restart, cancellation serialization, and single-flight regressions. |
