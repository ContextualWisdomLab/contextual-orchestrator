...
| The 2026-08-13 loopback MLX probe doubled eight-request 3B batch throughput from 0.431 to 0.983 requests/second at `local_concurrency=2`, while `4` reached only 0.987 and added no material gain. | Keep interactive route/conduct sequential; recommend an explicit batch starting point of `local_concurrency=2`, retain the bounded knob, and re-measure per model/server instead of silently raising the global default. | Observed and recorded in `docs/benchmarks/2026-08-13-local-mlx-gateway.md`; no quality claim |
