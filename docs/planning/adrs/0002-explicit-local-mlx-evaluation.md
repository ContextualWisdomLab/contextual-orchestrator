...
* `contextual_orchestrator/batch_routing.py` and `contextual_orchestrator/cost_router.py`: reuse the bounded `ModelClient.local_concurrency` value for the default in-process batch backend; keep the standalone default at one and preserve request ordering and fail-fast errors.
