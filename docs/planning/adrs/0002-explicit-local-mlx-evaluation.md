...
| A conducted request expanded a large discovered catalog and attempted unprobed providers while trying to rank workflow roles, exhausting the caller deadline before a viable structured member was established. | Admission and discovery are separate: a conducted request may rank and call only members with current structured-readiness evidence (deterministic mock transports are the test-only exception). An empty admitted set returns typed `no_viable_agent` without a provider completion. Broad readiness refresh belongs to an explicit, bounded, asynchronous administrative job; it never runs inside discovery or a buyer request. | Request-path admission implemented with 455-unprobed and two-ready failure bounds; durable administrative refresh remains a separately tracked follow-up. |
