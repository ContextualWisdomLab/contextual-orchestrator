...
| A conducted request expanded a large discovered catalog and attempted unprobed providers while trying to rank workflow roles, exhausting the caller deadline before a viable structured member was established. | Admission and discovery are separate: a conducted request may rank and call only members with successful structured-readiness evidence (deterministic mock transports are the test-only exception). The latest successful evidence remains authoritative until an actual provider or circuit failure records contradictory evidence; an arbitrary wall-clock freshness interval is not evidence. An empty admitted set returns typed `no_viable_agent` without a provider completion. Broad readiness refresh belongs to an explicit, bounded, asynchronous administrative job; it never runs inside discovery or a buyer request. The job requires an explicit agent access list and capability, uses the existing durable registry and provider-owned concurrency, is single-flight by canonical scope, validates the provider timeout before submission, serializes cancellation against progress and admission publication, supports deadline/cancellation/progress, and restores completed structured evidence after restart. Its execution claim uses the Valkey token as a compare-and-set owner and renews that owner's lease through the persisted deadline; release-time ownership loss cannot replace an already persisted terminal result. | Request-path admission and durable administrative refresh implemented with 455-unprobed, two-ready failure-bound and invalidation, seven-candidate lease renewal, restart, cancellation-race, timeout-bound, and single-flight regressions. |
