...
After this PR, central PR #1170, and target PR #790 are merged through their
protected paths, enable Actions for this repository. The first scheduled run
will inspect one current PR at `07` past the next hour. Use the normal
protected scheduler path for subsequent runs; this caller intentionally has no
manual branch-selection entry point.
