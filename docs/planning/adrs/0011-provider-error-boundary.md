...
`tests/test_model_discovery.py` and `tests/test_provider_reliability.py` assert
that provider response text is absent from public messages and causes, while
the full suite must remain green before merge.
