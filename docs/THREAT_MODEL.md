...
| Malicious provider response | Oversized, malformed, duplicate-key, non-finite, or deceptive response exhausts or contaminates runtime. | Body/output/time bounds and validation seams. | Complete strict framing/JSON/SSE boundary is `active_pr` #96. |
...
| Denial of wallet/service | Large prompts, deep workflows, retries, concurrency, or batch fan-out exhaust resources. | Body, output, workflow-step, retry, rate, concurrency, cache, and budget bounds. | Distributed quotas and provider-level reservation are deployment concerns. |
...
| PII overexposure | Full traces or persisted payloads reach a broader audience than the inference request. | Trace is not exposed by default; admin/inference authority can be split; redaction/minimization. | Fine-grained tenant RBAC and subject-rights workflow are host-owned. |
...
counts, not the message body. An operator without trace authority cannot retrieve
the full prompt. This preserves business utility without making broad copies.
