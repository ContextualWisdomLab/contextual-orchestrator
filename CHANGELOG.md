...
- Run the full test suite inside a digest-pinned Rust 1.97.1/Python 3.12 test
  runner with hash-locked requirements, so git-backed `fast-mlsirm`, its
  `numpy` dependency, and the Rust counting authority are installed in CI and
  locally.
